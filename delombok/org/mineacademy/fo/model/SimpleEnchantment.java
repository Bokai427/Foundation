...
public abstract class SimpleEnchantment extends Enchantment {
	...
	private static NamespacedKey toKey(@NonNull String name) {
		...
		if (!MinecraftVersion.atLeast(V.v1_13)) throw new RuntimeException("SimpleEnchantment requires Minecraft 1.13.2 or greater. Cannot make " + name);
	...
	public static Map<SimpleEnchantment, Integer> findEnchantments(ItemStack item) {
		...
		try {
			...
			if (Remain.hasItemMeta()) err.printStackTrace();
			return map;
		...
		for (final Entry<Enchantment, Integer> e : vanilla.entrySet()) {
			...
			if (enchantment instanceof SimpleEnchantment) map.put((SimpleEnchantment) enchantment, level);
		}
		return map;
	}
	...
	public static ItemStack addEnchantmentLores(ItemStack item) {
		...
		try {
			for (final Map.Entry<Enchantment, Integer> e : item.getEnchantments().entrySet()) if (e.getKey() instanceof SimpleEnchantment) {
				final String lore = ((SimpleEnchantment) e.getKey()).getLore(e.getValue());
				if (lore != null && !lore.isEmpty()) customEnchants.add(Common.colorize("&r&7" + lore));
			}
		} catch (final NullPointerException ex) {
		}
		...
		if (!customEnchants.isEmpty()) {
			...
			for (final String original : originalLore) colorlessOriginals.add(ChatColor.stripColor(Common.colorize(original)));
			// Place our enchants
			for (final String customEnchant : customEnchants) {
				...
				if (!colorlessOriginals.contains(colorlessEnchant)) finalLore.add(customEnchant);
			}
