...
public final class BoxedMessage {
	...
	public BoxedMessage(@NonNull String... messages) {
		...
		if (messages == null) {
			throw new java.lang.NullPointerException("messages is marked non-null but is null");
		}
	}
	...
	private void sendFrameInternals0() {
		for (int i = 0; i < this.getTopLines(); i++) this.send("&r");
		for (final String message : this.messages) for (final String part : message.split("\n")) this.send(part);
		for (int i = 0; i < this.getBottomLines(); i++) this.send("&r");
	}
	...
	private int length() {
		int length = 0;
		for (final String message : this.messages) for (@SuppressWarnings("unused") final String part : message.split("\n")) length++;
		return length;
	}

	private void send(String message) {
		message = this.centerMessage0(message);
		if (this.recipients == null) this.broadcast0(message);
		 else this.tell0(message);
	}

	private String centerMessage0(String message) {
		if (message.startsWith("<center>")) return ChatUtil.center(message.replaceFirst("\\<center\\>(\\s|)", ""));
		return message;
	}

	private void broadcast0(String message) {
		if (this.sender != null) Common.broadcast(message, this.sender);
		 else Common.broadcastTo(Remain.getOnlinePlayers(), message);
	}

	private void tell0(String message) {
		if (this.sender != null) message = message.replace("{player}", Common.resolveSenderName(this.sender));
	...
	public class Replacor {
		...
		public Replacor(final String[] variables) {
