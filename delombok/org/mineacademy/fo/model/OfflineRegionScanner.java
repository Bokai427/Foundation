...
public abstract class OfflineRegionScanner {
	...
	private void scanFile(File file, Queue<File> queue) {
		...
		for (int x = 0; x < 32; x++) for (int z = 0; z < 32; z++) {
			...
			if (RegionAccessor.isChunkSaved(region, x, z)) if (this.fastMode) this.onChunkScanFast(chunkX, chunkZ);
			 else {
		...
		if (this.fastMode) this.schedule0(queue);
		 else Common.runLater(WAIT_TIME_BETWEEN_SCAN_SECONDS, () -> this.schedule0(queue));
	}
	...
	private void finishScan() {
		if (this.watchdog != null) this.watchdog.resume();
		this.onScanFinished();
	}
	...
	public static int getEstimatedWaitTimeSec(World world) {
		final File[] files = getRegionFiles(world);
		return (int) (Math.round(WAIT_TIME_BETWEEN_SCAN_SECONDS * 1.5) * files.length);
	}
...
class RegionAccessor {
	...
	static {
		...
		try {
			...
			regionFileConstructor = atleast1_18 ? regionFileClass.getConstructor(Path.class, Path.class, boolean.class) : atleast1_16 ? regionFileClass.getConstructor(File.class, File.class, boolean.class) : atleast1_15 ? regionFileClass.getConstructor(File.class, File.class) : regionFileClass.getConstructor(File.class);
