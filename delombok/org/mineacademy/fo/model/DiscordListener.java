...
public abstract class DiscordListener implements Listener {
	...
	public void register() {
		if (!registeredListeners.contains(this)) registeredListeners.add(this);
	}
	...
	protected final TextChannel findChannel(long channelId) {
		...
		if (jda != null) return jda.getTextChannelById(channelId);
		return null;
	}
	...
	protected final List<TextChannel> findChannels(String channelName) {
		...
		if (jda != null) {
			...
			for (final TextChannel channel : jda.getTextChannels()) if (channel.getName().equalsIgnoreCase(channelName)) channels.add(channel);
			return channels;
		}
	...
	protected final void checkBoolean(boolean value, String warningMessage) throws RemovedMessageException {
		if (!value) this.removeAndWarn(warningMessage);
	}
	...
	protected final boolean hasRole(Member member, String roleName) {
		for (final Role role : member.getRoles()) if (role.getName().equalsIgnoreCase(roleName)) return true;
		return false;
	}
	...
	protected final void sendWebhookMessage(@Nullable CommandSender sender, String channelName, String message) {
		...
		Common.runAsync(() -> {
			try {
				...
				if (sender instanceof Player) WebhookUtil.deliverMessage(channel, (Player) sender, message);
				 else HookManager.sendDiscordMessage(sender, channelName, message);
		...
		Common.runAsync(() -> {
			...
			Common.runLaterAsync(4 * 20, () -> {
				try {
					...
					if (!ex.getMessage().contains("Unknown Message")) ex.printStackTrace();
				}
			});
		});
	}
	...
	protected final void deleteMessageById(TextChannel channel, long messageId) {
		Common.runAsync(() -> {
			...
			try {
				...
				if (!(t instanceof github.scarsz.discordsrv.dependencies.jda.api.exceptions.ErrorResponseException)) t.printStackTrace();
				 else Debugger.debug("discord", "Could not remove Discord message in channel \'" + channel.getName() + "\' id " + latestMessageId + ", it was probably deleted otherwise or this is a bug.");
			}
		});
	}
		...
		Common.runAsync(() -> {
			try {
				...
				if (message != null) {
					...
					// Send a new one
					final Message newSentMessage = channel.sendMessage(format.replace("{player}", message.getAuthor().getName())).complete();
					this.editedMessages.put(messageId, newSentMessage.getIdLong());
				}
			} catch (final Throwable t) {
				if (!t.toString().contains("Unknown Message")) t.printStackTrace();
			}
		});
	}
	...
	protected final String findPlayerName(Member member, User author) {
		...
		player = Remain.getPlayerByUUID(linkedId);
		 else player = Bukkit.getPlayer(discordName);
	...
	public final void kickMember(DiscordSender discordSender, String reason) {
		Common.runAsync(() -> {
			try {
				...
				if (member != null) member.kick(reason).complete();
	...
	public static final class DiscordListenerImpl implements Listener {
		...
		public void onMessageReceived(DiscordGuildMessagePreProcessEvent event) {
			for (final DiscordListener listener : registeredListeners) try {
		...
		public static DiscordListenerImpl getInstance() {
			return DiscordListenerImpl.instance;
		}
	}
