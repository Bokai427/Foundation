...
public final class ChatImage {
	...
	public ChatImage appendText(String... text) {
		for (int y = 0; y < this.lines.length; y++) if (text.length > y) {
	...
	private String center(String message, int length) {
		if (message.length() > length) return message.substring(0, length);
		 else if (message.length() == length) return message;
		 else {
	...
	public void sendToPlayer(CommandSender sender) {
		for (final String line : this.lines) sender.sendMessage(Variables.replace(line, sender));
	}
	...
	public static ChatImage fromFile(@NonNull File file, int height, Type characterType) throws IOException {
		...
		if (image == null) throw new NullPointerException("Unable to load image size " + file.length() + " bytes from " + file.toPath());
		 else {
	...
	private static CompChatColor[][] parseImage(BufferedImage newImage, int height) {
		...
		for (int x = 0; x < resized.getWidth(); x++) for (int y = 0; y < resized.getHeight(); y++) {
