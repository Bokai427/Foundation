...
public class SpigotUpdater implements Runnable {
	...
	public void run() {
		...
		if (!this.canUpdateFrom(currentVersion)) return;
		try {
			...
			if (this.newVersion.isEmpty()) return;
			if (this.isNewerVersion(currentVersion, this.newVersion) && this.canUpdateTo(this.newVersion)) {
				...
				} else Common.log(this.getNotifyMessage());
			}
			...
			// no permission
			if (ex.getMessage().startsWith("Server returned HTTP response code:")) Common.log("Could not check for update, SpigotMC site appears to be down (or unaccessible): " + ex.getMessage());
			 else Common.error(ex, "IOException performing update from SpigotMC.org check for " + SimplePlugin.getNamed());
	...
	private boolean isNewerVersion(final String current, final String remote) {
		if (remote.contains("-LEGACY")) return false;
		...
		if (currParts.length != remoteParts.length) {
			...
			for (int i = 0; i < (olderIsLonger ? currParts.length : remoteParts.length); i++) modifiedParts[i] = olderIsLonger ? remoteParts.length > i ? remoteParts[i] : "0" : currParts.length > i ? currParts[i] : "0";
			if (olderIsLonger) remoteParts = modifiedParts;
			 else currParts = modifiedParts;
		}
		for (int i = 0; i < currParts.length; i++) {
			if (Integer.parseInt(currParts[i]) > Integer.parseInt(remoteParts[i])) return false;
			if (Integer.parseInt(remoteParts[i]) > Integer.parseInt(currParts[i])) return true;
		}
		return false;
	}
