...
public abstract class PacketListener {
	...
	protected void addReceivingListener(final ListenerPriority priority, final PacketType type, final Consumer<PacketEvent> consumer) {
		this.addPacketListener(new SimpleAdapter(priority, type) {
			...
			public void onPacketReceiving(final PacketEvent event) {
				if (event.getPlayer() != null) consumer.accept(event);
			}
		});
	}
	...
	protected abstract class SimpleChatAdapter extends SimpleAdapter {
		...
		public void onPacketSending(final PacketEvent event) {
			...
			if (!this.player.isOnline() || SimplePlugin.isReloading()) return;
			// Prevent deadlock
			if (this.processedPlayers.contains(playerName)) return;
			...
			try {
				...
				if (this.jsonMessage != null && !this.jsonMessage.isEmpty()) this.onJsonMessage(this.jsonMessage);
				if (!Common.stripColors(legacyText).equals(Common.stripColors(parsedText))) this.writeEditedMessage(parsedText, event);
			} finally {
		...
		private String compileChatMessage(PacketEvent event) {
			...
			if (MinecraftVersion.atLeast(V.v1_7)) {
				if (this.systemChat) this.jsonMessage = event.getPacket().getStrings().read(0);
				 else {
					...
					if (component != null) this.jsonMessage = component.getJson();
					 else 
					...
					if (packet.size() > 1) {
						...
						if (secondField == null) {
							...
							if (secondField != null) this.adventure = true;
						}
			...
			} else this.jsonMessage = event.getPacket().getStrings().read(0);
			...
			if (this.jsonMessage.length() < 50000) {
		...
		private void writeEditedMessage(String message, PacketEvent event) {
			...
			if (this.systemChat) event.getPacket().getStrings().writeSafely(0, this.jsonMessage);
			 else if (this.isBaseComponent) packet.writeSafely(this.adventure ? 2 : 1, Remain.toComponent(this.jsonMessage));
			 else if (MinecraftVersion.atLeast(V.v1_7)) event.getPacket().getChatComponents().writeSafely(0, WrappedChatComponent.fromJson(this.jsonMessage));
			 else event.getPacket().getStrings().writeSafely(0, SerializedMap.of("text", this.jsonMessage.substring(1, this.jsonMessage.length() - 1)).toJson());
		}
