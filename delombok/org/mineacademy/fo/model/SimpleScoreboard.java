...
public class SimpleScoreboard {
	...
	public static void clearBoardsFor(final Player player) {
		for (final SimpleScoreboard scoreboard : registeredBoards) if (scoreboard.isViewing(player)) scoreboard.hide(player);
	}
	 ...
	 * @return
	 */
	protected String replaceVariables(@NonNull final Player player, @NonNull final String message) {
		if (player == null) {
	...
	public final void show(final Player player) {
		...
		if (this.updateTask == null) this.start();
	...
	public final void hide(final Player player) {
		...
		if (this.viewers.isEmpty()) this.cancelUpdateTask();
	}
	...
	public final void setTheme(@NonNull final ChatColor primary, final ChatColor secondary) {
		if (primary == null) {
			throw new java.lang.NullPointerException("primary is marked non-null but is null");
		}
		if (secondary != null) {
			this.theme[0] = "&" + primary.getChar();
			this.theme[1] = "&" + secondary.getChar();
		} else this.theme[0] = "&" + primary.getChar();
	}
	...
	public final void addRows(final List<Object> entries) {
		...
		for (Object object : entries) lines.add(object == null ? "" : Common.colorize(SerializeUtil.serialize(Mode.YAML, object).toString()));
	...
	private void start() {
		...
		this.updateTask = Bukkit.getScheduler().runTaskTimer(SimplePlugin.getInstance(), () -> {
			try {
				...
				Common.error(t, "Error displaying " + this, "Entries: " + this.rows, "Title: " + this.title, "%error", "Stopping rendering for safety.");
				this.stop();
			}
	...
	public final void stop() {
		this.viewers.forEach(viewerId -> {
			final Player viewer = Bukkit.getPlayer(viewerId);
			if (viewer != null && viewer.isOnline()) viewer.setScoreboard(Bukkit.getScoreboardManager().getMainScoreboard());
		});
		this.viewers.clear();
		if (this.updateTask != null) this.cancelUpdateTask();
	}
	...
	private String replaceTheme(final String row) {
		if (row.contains(":")) if (this.theme.length == 1) return this.theme[0] + row;
		 else if (this.theme[0] != null) {
			final String[] split = row.split("\\:");
			if (split.length > 1) return this.theme[0] + split[0] + ":" + this.theme[1] + split[1];
		}
		return row;
	}
	...
	private void reloadEntries(Player player) throws IllegalArgumentException {
		...
		if (!mainboard.getDisplayName().equals(this.title)) mainboard.setDisplayName(this.title);
		for (int lineNumber = 0; lineNumber < 16; lineNumber++) {
			...
			if (lineNumber < this.rows.size()) {
				if (line == null) line = scoreboard.registerNewTeam("line" + scoreboardLineNumber);
				...
				if (!line.getPrefix().equals(prefix)) line.setPrefix(prefix);
				if (line.getEntries().size() > 1) throw new IllegalArgumentException("There are more than allowed entries in the teams entry list!");
				...
				if (!line.getSuffix().equals(suffix)) line.setSuffix(suffix);
				if (oldEntry != null) scoreboard.resetScores(oldEntry);
				mainboard.getScore(entry).setScore(scoreboardLineNumber);
			} else if (line != null) {
				for (final String oldEntry : line.getEntries()) scoreboard.resetScores(oldEntry);
				line.unregister();
			}
		}
	}
	...
	private static List<String> copyColors(String text, int maxLength, int... splitPoints) {
		if (maxLength < 1) return new ArrayList<>();
		List<String> splitText = new ArrayList<>();
		int lastSplitPoint = 0;
		for (final int splitPoint : splitPoints) {
			if (splitPoint < 0) throw new IllegalArgumentException("SplitPoints cannot be negative!");
			 else if (splitPoint <= lastSplitPoint) throw new IllegalArgumentException("Please put your splitPoints in order from small to large!");
			 else if (splitPoint >= text.length() || splitPoint >= maxLength) break;
			splitText.add(text.substring(lastSplitPoint, splitPoint));
			lastSplitPoint = splitPoint;
		}
		if (text.length() > lastSplitPoint) splitText.add(text.substring(lastSplitPoint, Math.min(text.length(), maxLength)));
		for (int index = 1; index < splitText.size(); index++) {
			...
			for (int j = index + 1; j < splitText.size(); j++) linesWithMarge.add(splitText.get(j));
			linesWithMarge = passMergeThrough(marge, linesWithMarge, maxLength);
			for (int j = 0; j < linesWithMarge.size(); j++) splitText.set(index + 1 + j, linesWithMarge.get(j));
		}
		int stringLength = 0;
		for (final String line : splitText) stringLength += line.length();
		if (stringLength > maxLength) splitText = passMergeThrough(null, splitText, maxLength);
		return splitText;
	}
	...
	public static List<SimpleScoreboard> getRegisteredBoards() {
		return SimpleScoreboard.registeredBoards;
	}
