...
public final class RangedValue {
	...
	public static RangedValue parse(String line) {
		...
		if (split.length == 1) parts = new String[] {(firstNegative ? "-" : "") + line};
		 else parts = new String[] {(firstNegative ? "-" : "") + split[0], (secondNegative ? "-" + split[2] : split[1])};
		...
		Valid.checkBoolean(Valid.isNumber(first), "Invalid ranged value 1. input: \'" + first + "\' from line: \'" + line + "\'. RangedValue no longer accepts human natural format, for this, use RangedSimpleTime instead.");
		Valid.checkBoolean(Valid.isNumber(second), "Invalid ranged value 2. input: \'" + second + "\' from line: \'" + line + "\'. RangedValue no longer accepts human natural format, for this, use RangedSimpleTime instead.");
		...
		// Check if 1<2
		if (first.contains(".")) Valid.checkBoolean(firstNumber.longValue() <= secondNumber.longValue(), "First number cannot be greater than second: " + firstNumber.longValue() + " vs " + secondNumber.longValue() + " in " + line);
		 else Valid.checkBoolean(firstNumber.doubleValue() <= secondNumber.doubleValue(), "First number cannot be greater than second: " + firstNumber.doubleValue() + " vs " + secondNumber.doubleValue() + " in " + line);
