...
public final class GeoAPI {
	...
	public static GeoResponse getCountry(InetSocketAddress ip) {
		...
		if (ip == null) return response;
		if (ip.getHostString().equals("127.0.0.1") || ip.getHostString().equals("0.0.0.0")) return new GeoResponse("local", "-", "local", "-");
		if (cache.containsKey(ip.toString()) || cache.containsValue(response)) return cache.get(ip.toString());
		try {
			...
			try (BufferedReader r = new BufferedReader(new InputStreamReader(con.getInputStream()))) {
				String page = "";
				String input;
				while ((input = r.readLine()) != null) page += input;
	...
	public static final class GeoResponse {
		...
		public GeoResponse(final String countryName, final String countryCode, final String regionName, final String isp) {
