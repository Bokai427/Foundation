...
public final class VisualizedRegion extends Region {
	...
	public void showParticles(Player player, @Nullable Color color, int durationTicks) {
		...
		Common.runLater(durationTicks, () -> {
			if (this.canSeeParticles(player)) this.hideParticles(player);
		});
	}
	...
	public void showParticles(final Player player, @Nullable Color color) {
		...
		if (this.task == null) this.startVisualizing();
	}
	...
	public void hideParticles(final Player player) {
		...
		if (this.viewers.isEmpty() && this.task != null) this.stopVisualizing();
	}
	...
	private void startVisualizing() {
		...
		this.task = Common.runTimer(this.delayTicks, new BukkitRunnable() {
			...
			public void run() {
				...
				for (final Location location : blocks) for (final Map.Entry<Player, Color> entry : VisualizedRegion.this.viewers.entrySet()) {
					final Player viewer = entry.getKey();
					final Color color = entry.getValue();
					final Location viewerLocation = viewer.getLocation();
					if (viewerLocation.getWorld().equals(location.getWorld()) && viewerLocation.distance(location) < 100) if (color != null) CompParticle.REDSTONE.spawn(viewer, location, color, 0.5F);
					 else VisualizedRegion.this.particle.spawn(viewer, location);
				}
			}
		});
	}
	...
	public static VisualizedRegion deserialize(final SerializedMap map) {
		...
		if (map.isEmpty()) return new VisualizedRegion();
	 ...
	 */
	@java.lang.SuppressWarnings("all")
	public void setParticle(final CompParticle particle) {
	 ...
	 */
	@java.lang.SuppressWarnings("all")
	public void setDelayTicks(final int delayTicks) {
