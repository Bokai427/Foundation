...
public abstract class VisualTool extends BlockTool {
	...
	protected void handleBlockClick(Player player, ClickType click, Block block) {
		...
		if (region != null) {
			...
			if (!isPrimary && region.hasPrimary() && region.isPrimary(location)) region.setPrimary(null);
			// ...and vice versa
			if (isPrimary && region.hasSecondary() && region.isSecondary(location)) region.setSecondary(null);
	...
	protected List<Location> getVisualizedPoints(Player player) {
		...
		if (region != null) {
			if (region.hasPrimary()) points.add(region.getPrimary());
			if (region.hasSecondary()) points.add(region.getSecondary());
		}
		return points;
	}
	...
	private void visualize(@NonNull final Player player) {
		...
		if (region != null && region.isWhole()) if (!region.canSeeParticles(player)) region.showParticles(player);
		for (final Location location : this.getVisualizedPoints(player)) {
			if (location == null) continue;
			final Block block = location.getBlock();
			if (!BlockVisualizer.isVisualized(block)) BlockVisualizer.visualize(block, this.getBlockMask(block, player), this.getBlockName(block, player));
		}
	}
	...
	private void stopVisualizing(@NonNull final Player player) {
		...
		final VisualizedRegion region = this.getVisualizedRegion(player);
		if (region != null && region.canSeeParticles(player)) region.hideParticles(player);
		for (final Location location : this.getVisualizedPoints(player)) {
			if (location == null) continue;
			final Block block = location.getBlock();
			if (BlockVisualizer.isVisualized(block)) BlockVisualizer.stopVisualizing(block);
		}
	}
