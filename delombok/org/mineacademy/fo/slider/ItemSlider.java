...
public final class ItemSlider implements Slider<List<ItemStack>> {
	...
	public List<ItemStack> next() {
		if (this.currentPointer == this.width) this.currentPointer = 0;
		final List<ItemStack> items = new ArrayList<>();
		for (int i = this.width - 1; i > this.width - this.currentPointer - 1; i--) items.add(this.fillerItem);
		items.add(this.highlightItem);
		for (int i = 0; i < this.width - this.currentPointer - 1; i++) items.add(this.fillerItem);
		this.currentPointer++;
		return items;
	}
	...
	private ItemSlider(final ItemStack fillerItem, final ItemStack highlightItem) {
