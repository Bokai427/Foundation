...
public final class ItemFrameBounceSlider implements Slider<Map<Integer, ItemStack>> {
	...
	public Map<Integer, ItemStack> next() {
		...
		for (int index = 0; index < this.frameSize; index++) {
			...
			if (row == 0 || row == rowCount - 1) items.put(index, this.fillerItem);
			if (column == 1 || column == 9) items.put(index, this.sideItem);
		}
		if (this.topDecreasing) this.topPointer--;
		 else this.topPointer++;
		if (this.topPointer == 0) this.topDecreasing = false;
		 else if (this.topPointer == 8) this.topDecreasing = true;
		if (this.bottomDecreasing) this.bottomPointer--;
		 else this.bottomPointer++;
		if (this.bottomPointer == this.frameSize - 9) this.bottomDecreasing = false;
		 else if (this.bottomPointer == this.frameSize - 1) this.bottomDecreasing = true;
		items.replace(this.topPointer, this.highlightItem);
		items.replace(this.bottomPointer, this.highlightItem);
		if (this.topPointer == 0 || this.topPointer == 8) items.replace(this.topPointer, this.cornerItem);
		if (this.bottomPointer == this.frameSize - 9 || this.bottomPointer == this.frameSize - 1) items.replace(this.bottomPointer, this.cornerItem);
		return items;
	}
	...
	private ItemFrameBounceSlider(final ItemStack fillerItem, final ItemStack highlightItem, final ItemStack sideItem, final ItemStack cornerItem) {
