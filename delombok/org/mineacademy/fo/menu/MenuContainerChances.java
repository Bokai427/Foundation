...
public abstract class MenuContainerChances extends Menu implements MenuQuantitable {
	...
	public final ItemStack getItemAt(int slot) {
		if (slot == this.getChangeModeButtonPosition()) return this.changeModeButton.getItem();
		final ItemStack customDrop = this.getDropAt(slot);
		if (customDrop != null) {
			if (this.mode == EditMode.ITEM || !this.canEditItem(slot)) return customDrop;
		...
		if (slot > this.getSize() - 9) return MenuContainer.BOTTOM_BAR_FILLER_ITEM;
		return NO_ITEM;
	}
	...
	public final boolean isActionAllowed(final MenuClickLocation location, final int slot, final ItemStack clicked, final ItemStack cursor, InventoryAction action) {
		if (this.mode == EditMode.CHANCE) return false;
		if (location != MenuClickLocation.MENU) return true;
		if (!this.canEditItem(location, slot, clicked, cursor, action)) return false;
	...
	protected final void onMenuClick(Player player, int slot, InventoryAction action, ClickType click, ItemStack cursor, ItemStack clicked, boolean cancelled) {
		if (this.mode == EditMode.CHANCE && this.canEditItem(slot) && slot < this.getSize() - 9) {
			...
			final double newChance = MathUtil.range(chance + next, 0.0, 1.0);
			// Save drop chance
	...
	protected String[] getInfo() {
		if (this.mode == EditMode.ITEM) return new String[] {"This menu allows you to drop", "items to this container.", "", "Simply &2drag and drop &7items", "from your inventory here."};
		 else return new String[] {"This menu allows you to edit drop", "chances for items in this container.", "", "&2Right or left click &7on items", "to adjust their drop chance."};
	}
	...
	public enum EditMode {
		...
		private EditMode(final String key) {
			this.key = key;
		}
	 ...
	 */
	@java.lang.SuppressWarnings("all")
	public void setQuantity(final MenuQuantity quantity) {
