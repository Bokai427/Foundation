	...
	final void registerButtons() {
		...
		// Register buttons explicitly given
		{
			...
			if (buttons != null) {
				...
				for (final Button button : buttons) buttonsRemapped.put(button, null);
		...
		// Register buttons declared as fields
		{
			...
			do for (final Field f : lookup.getDeclaredFields()) this.registerButton0(f);
 while (Menu.class.isAssignableFrom(lookup = lookup.getSuperclass()));
	...
	private void registerButton0(final Field field) {
		...
		if (Button.class.isAssignableFrom(type)) {
			...
			if (!(button instanceof DummyButton)) this.registeredButtons.put(button, position);
		} else if (Button[].class.isAssignableFrom(type)) throw new FoException("Button[] is no longer supported in menu for " + this.getClass());
	}
	...
	protected final Button getButton(final ItemStack fromItem) {
		...
		for (final Button button : this.registeredButtons.keySet()) {
			...
			if (ItemUtil.isSimilar(fromItem, item)) return button;
		}
		return null;
	}
	...
	public Menu newInstance() {
		try {
			...
			try {
				...
				if (parent != null) return ReflectionUtil.instantiate(this.getClass(), parent);
	...
	public final void displayTo(final Player player) {
		...
		if (MinecraftVersion.olderThan(V.v1_5)) {
			...
			if (Messenger.ENABLED) Messenger.error(player, error);
			 else Common.tell(player, error);
			return;
		}
		this.viewer = player;
		this.registerButtonsIfHasnt();
		...
		if (!Common.callEvent(new MenuOpenEvent(this, drawer, player))) return;
		...
		// Register previous menu if exists
		{
			...
			if (previous != null) player.setMetadata(FoConstants.NBT.TAG_MENU_PREVIOUS, new FixedMetadataValue(SimplePlugin.getInstance(), previous));
		}
	...
	private void debugSlotNumbers(final InventoryDrawer drawer) {
		if (this.slotNumbersVisible) for (int slot = 0; slot < drawer.getSize(); slot++) {
			final ItemStack item = drawer.getItem(slot);
			if (item == null) drawer.setItem(slot, ItemCreator.of(CompMaterial.LIGHT_GRAY_STAINED_GLASS_PANE, "Slot " + slot).make());
		}
	}
	...
	public final void restartMenu(final String animatedTitle) {
		...
		for (int i = 0; i < content.length; i++) content[i] = newContent.get(i);
		inventory.setContents(content);
		if (animatedTitle != null) this.animateTitle(animatedTitle);
	}
	...
	private Map<Integer, ItemStack> compileItems() {
		...
		for (int slot = 0; slot < this.size; slot++) {
			...
			if (item != null && CompMaterial.isAir(item)) item = null;
			items.put(slot, item);
		}
		// Override by buttons
		for (final Map.Entry<Button, Position> entry : this.registeredButtons.entrySet()) {
			...
			if (startPosition == StartPosition.CENTER) slot += this.getCenterSlot();
			 else if (startPosition == StartPosition.BOTTOM_CENTER) slot += this.getSize() - 5;
			 else if (startPosition == StartPosition.BOTTOM_LEFT) slot += this.getSize() - (hasReturnButton ? 2 : 1);
			 else if (startPosition == StartPosition.TOP_LEFT) slot += 0;
			 else throw new FoException("Does not know how to implement button position\'s Slot." + startPosition);
		...
		if (this instanceof MenuQuantitable) {
			...
			if (slot != -1) items.put(slot, this.quantityButton.getItem());
		}
		// Override by hotbar
		{
			if (this.addInfoButton() && this.getInfo() != null) items.put(this.getInfoButtonPosition(), Button.makeInfo(this.getInfo()).getItem());
			if (hasReturnButton) items.put(this.getReturnButtonPosition(), this.returnButton.getItem());
		}
		return items;
	}
	...
	public final void animateTitle(final String title) {
		if (titleAnimationEnabled) PlayerUtil.updateInventoryTitle(this, this.getViewer(), title, this.getTitle(), titleAnimationDurationTicks);
	}
	...
	protected final void setTitle(final String title) {
		this.title = title;
		if (this.viewer != null && this.opened) PlayerUtil.updateInventoryTitle(this.viewer, title);
	}
	...
	protected final void setViewer(@NonNull final Player viewer) {
		if (viewer == null) {
			throw new java.lang.NullPointerException("viewer is marked non-null but is null");
		}
		this.viewer = viewer;
	}
	 ...
	 */
	@java.lang.SuppressWarnings("all")
	public static void setSound(@Nullable final SimpleSound sound) {
		Menu.sound = sound;
	}
	...
	public static boolean isTitleAnimationEnabled() {
		return Menu.titleAnimationEnabled;
	}
	 ...
	 */
	@java.lang.SuppressWarnings("all")
	public static void setTitleAnimationEnabled(final boolean titleAnimationEnabled) {
	 ...
	 */
	@java.lang.SuppressWarnings("all")
	public static void setTitleAnimationDurationTicks(final int titleAnimationDurationTicks) {
