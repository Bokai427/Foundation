...
public abstract class MenuPagged<T> extends Menu {
	...
	public Button formPreviousButton() {
		return new Button() {
			...
			public void onClickedInMenu(final Player player, final Menu menu, final ClickType click) {
				if (this.canGo) setCurrentPage(MathUtil.range(getCurrentPage() - 1, 1, getPages().size()));
			}
			@Override
			public ItemStack getItem() {
				final int previousPage = getCurrentPage() - 1;
				return ItemCreator.of(this.canGo ? MenuPagged.getActivePageButton() : MenuPagged.getInactivePageButton()).name(previousPage == 0 ? SimpleLocalization.Menu.PAGE_FIRST : SimpleLocalization.Menu.PAGE_PREVIOUS.replace("{page}", String.valueOf(previousPage))).make();
			}
		};
	}
	...
	public Button formNextButton() {
		return new Button() {
			...
			public ItemStack getItem() {
				...
				return ItemCreator.of(this.canGo ? MenuPagged.getActivePageButton() : MenuPagged.getInactivePageButton()).name(lastPage ? SimpleLocalization.Menu.PAGE_LAST : SimpleLocalization.Menu.PAGE_NEXT.replace("{page}", String.valueOf(getCurrentPage() + 1))).make();
			}
		};
	}
	...
	public ItemStack getItemAt(final int slot) {
		...
		if (slot == this.getPreviousButtonPosition()) return this.prevButton.getItem();
		if (slot == this.getNextButtonPosition()) return this.nextButton.getItem();
		return null;
	}
	...
	public final void onMenuClick(final Player player, final int slot, final InventoryAction action, final ClickType click, final ItemStack cursor, final ItemStack clicked, final boolean cancelled) {
		if (this.slots.contains(slot) && this.slots.indexOf(slot) < this.getCurrentPageItems().size()) {
			...
			if (obj != null) {
				final org.bukkit.event.inventory.InventoryType prevType = player.getOpenInventory().getType();
				this.onPageClick(player, obj, click);
				if (prevType == player.getOpenInventory().getType()) player.getOpenInventory().getTopInventory().setItem(slot, this.getItemAt(slot));
			}
		}
	}
	...
	public static CompMaterial getActivePageButton() {
		return MenuPagged.activePageButton;
	}
	 ...
	 */
	@java.lang.SuppressWarnings("all")
	public static void setActivePageButton(final CompMaterial activePageButton) {
	...
	public static CompMaterial getInactivePageButton() {
		return MenuPagged.inactivePageButton;
	}
	 ...
	 */
	@java.lang.SuppressWarnings("all")
	public static void setInactivePageButton(final CompMaterial inactivePageButton) {
