...
public final class TabUtil {
	...
	public static <T> List<String> complete(String partialName, T... elements) {
		...
		if (elements != null) for (final T element : elements) if (element != null) if (element instanceof Iterable) for (final Object iterable : (Iterable<?>) element) clone.add(iterable instanceof Enum ? iterable.toString().toLowerCase() : SerializeUtil.serialize(Mode.YAML, iterable).toString());
		 else 
		// Trick: Automatically parse enum constants
		if (element instanceof Enum[]) for (final Object iterable : ((Enum[]) element)[0].getClass().getEnumConstants()) clone.add(iterable.toString().toLowerCase());
		 else {
			final boolean lowercase = element instanceof Enum;
			final String parsed = SerializeUtil.serialize(Mode.YAML, element).toString();
			if (!"".equals(parsed)) clone.add(lowercase ? parsed.toLowerCase() : parsed);
		}
