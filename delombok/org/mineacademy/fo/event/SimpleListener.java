...
public abstract class SimpleListener<T extends Event> implements Listener, EventExecutor {
	...
	public final void execute(Listener listener, Event event) throws EventException {
		if (!event.getClass().equals(this.eventClass)) return;
		...
		try {
			...
			if (messages != null && player != null) for (String message : messages) {
				message = Variables.replace(message, player);
				if (Messenger.ENABLED) Messenger.error(player, message);
				 else Common.tell(player, "&c" + message);
			}
			if (cancelled && event instanceof Cancellable) ((Cancellable) event).setCancelled(true);
	...
	protected Player findPlayer() {
		...
		if (this.event instanceof PlayerEvent) return ((PlayerEvent) this.event).getPlayer();
	...
	protected final void checkBoolean(boolean condition, String... falseMessages) {
		if (!condition) throw new EventHandledException(true, falseMessages);
	}
	...
	protected final void checkPerm(String permission, String falseMessage) {
		...
		if (!PlayerUtil.hasPerm(player, permission)) throw new EventHandledException(true, falseMessage.replace("{permission}", permission));
	}
	...
	public SimpleListener(final Class<T> eventClass, final EventPriority priority, final boolean ignoreCancelled) {
