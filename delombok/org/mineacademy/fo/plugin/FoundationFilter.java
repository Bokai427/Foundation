...
final class FoundationFilter {
	...
	static void inject() {
		...
		for (final Plugin plugin : Bukkit.getPluginManager().getPlugins()) plugin.getLogger().setFilter(filter);
	...
	static boolean isFiltered(String message) {
		...
		// Log4j2 exploit
		if (message.contains("${jndi:ldap:")) return true;
		// Filter a warning since we've already patched this with NashornPlus extension
		if (message.equals("Warning: Nashorn engine is planned to be removed from a future JDK release")) return true;
		// One less spammy message for server owners
		if (message.endsWith("which is not a depend, softdepend or loadbefore of this plugin.")) return true;
		// Disable some annoying hikaripool or discordsrv messages
		if (message.contains("HikariPool-1 - Starting...") || message.contains("HikariPool-1 - Start completed.") || message.contains("[DiscordSRV] [JDA] Login Successful!") || message.contains("[DiscordSRV] [JDA] Connected to WebSocket")) return true;
		...
		if (SimplePlugin.hasInstance() && SimplePlugin.getInstance().getMainCommand() != null) {
			...
			if (message.contains("issued server command: /" + SimplePlugin.getInstance().getMainCommand().getLabel() + " internal") || message.contains("issued server command: /#flp")) return true;
			// Filter user-defined commands
			if (SimplePlugin.hasInstance()) for (String filter : SimplePlugin.getInstance().getConsoleFilter()) {
				filter = filter.toLowerCase();
				if (message.startsWith(filter) || message.contains(filter)) return true;
			}
		}
		return false;
	}
	...
	static void setMESSAGES_TO_FILTER(final List<String> MESSAGES_TO_FILTER) {
		FoundationFilter.MESSAGES_TO_FILTER = MESSAGES_TO_FILTER;
	}
}
...
class FilterSystem extends PrintStream {
	...
	public void println(Object x) {
		if (x != null && !FoundationFilter.isFiltered(x.toString())) super.println(x);
	}

	@Override
	public void println(String x) {
		if (x != null && !FoundationFilter.isFiltered(x)) super.println(x);
	}
}
