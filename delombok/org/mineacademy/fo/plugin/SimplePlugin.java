...
public abstract class SimplePlugin extends JavaPlugin implements Listener {
	...
	public static SimplePlugin getInstance() {
		if (instance == null) {
			try {
				...
				if (Bukkit.getPluginManager().getPlugin("PlugMan") != null) Bukkit.getLogger().severe("Failed to get instance of the plugin, if you reloaded using PlugMan you need to do a clean restart instead.");
				throw ex;
			}
	...
	public final void onLoad() {
		...
		try {
			...
			if (MinecraftVersion.olderThan(V.v1_7)) instance = this; // Workaround
			 else throw ex;
		}
		...
		if (this.suggestPaper() && !version.contains("Paper") && !version.contains("Purpur") && !version.contains("NachoSpigot") && !version.contains("-Spigot") && MinecraftVersion.atLeast(V.v1_8)) {
	...
	public final void onEnable() {
		...
		for (final StackTraceElement element : new Throwable().getStackTrace()) if (element.toString().contains("com.rylinaux.plugman.util.PluginUtil.load")) {
		...
		try {
			...
			if (!messenger.isOutgoingChannelRegistered(this, "BungeeCord")) messenger.registerOutgoingPluginChannel(this, "BungeeCord");
			...
			if (!this.isEnabled()) return;
			// Start update check
			if (this.getUpdateCheck() != null) this.getUpdateCheck().run();
			// Register our listeners
			this.registerEvents(this);
			this.registerEvents(new MenuListener());
			this.registerEvents(new FoundationListener());
			if (this.areToolsEnabled()) this.registerEvents(new ToolsListener());
			...
			if (this.getMetricsPluginId() != -1) new Metrics(this.getMetricsPluginId());
	...
	private void loadLibraries() {
		...
		if (!ReflectionUtil.isClassAvailable("net.md_5.bungee.api.ChatColor")) libraries.add(Library.fromMavenRepo("net.md-5", "bungeecord-chat", "1.16-R0.4"));
		if (MinecraftVersion.olderThan(V.v1_16)) {
			...
			// Load normally
			if (!libraries.isEmpty() && javaVersion >= 9) Common.logFramed("Warning: Unsupported Java version: " + javaVersion + " for your server", "version! Minecraft " + MinecraftVersion.getServerVersion() + " was designed for Java 8", "and we\'re unable unable to load \'legacy-libraries\'", "that this plugin uses:", Common.join(libraries, ", ", Library::getGroupId), "", "To fix this, start your server using Java 8 or", "upgrade to Minecraft 1.16 or greater.");
			 else for (final Library library : libraries) library.load();
		}
		// Always load user-defined libraries
		final List<Library> manualLibraries = this.getLibraries();
		// But only on Java 8 (for now)
		if (!manualLibraries.isEmpty() && javaVersion > 8) Common.warning("The getLibraries() feature only supports Java 8 for now and does not work on Java " + javaVersion + ". To load the following libraries, " + "install Java 8 or upgrade to Minecraft 16 where you use the \'libraries\' feature of plugin.yml to load. Skipping loading: " + manualLibraries);
		 else methodLibraryLoader:
		for (final Library library : manualLibraries) {
			// Detect conflicts
			for (final Library otherLibrary : libraries) if (library.getArtifactId().equals(otherLibrary.getArtifactId()) && library.getGroupId().equals(otherLibrary.getGroupId())) {
	...
	protected final void registerBungeeCord(@NonNull BungeeListener bungee) {
		if (bungee == null) {
			throw new java.lang.NullPointerException("bungee is marked non-null but is null");
		}
		final Messenger messenger = this.getServer().getMessenger();
		if (!messenger.isIncomingChannelRegistered(this, "BungeeCord")) messenger.registerIncomingPluginChannel(this, "BungeeCord", new BungeeListener.CommonBungeeListener());
	...
	private boolean checkServerVersions0() {
		...
		if (!MinecraftVersion.getCurrent().isTested()) Common.logFramed("*** WARNING ***", "Your Minecraft version " + MinecraftVersion.getCurrent() + " has not yet", "been officialy tested with the Foundation,", "the library that " + SimplePlugin.getNamed() + " plugin uses.", "", "Loading the plugin at your own risk...", Common.consoleLine());
		// Check min version
		final V minimumVersion = this.getMinimumVersion();
		if (minimumVersion != null && MinecraftVersion.olderThan(minimumVersion)) {
			Common.logFramed(false, this.getDataFolder().getName() + " requires Minecraft " + minimumVersion + " or newer to run.", "Please upgrade your server.");
			return false;
		}
		// Check max version
		final V maximumVersion = this.getMaximumVersion();
		if (maximumVersion != null && MinecraftVersion.newerThan(maximumVersion)) {
			Common.logFramed(false, this.getDataFolder().getName() + " requires Minecraft " + maximumVersion + " or older to run.", "Please downgrade your server or", "wait for the new version.");
			return false;
		}
		return true;
	}
	...
	protected final void displayError0(Throwable throwable) {
		...
		} else if (throwable instanceof UnsupportedOperationException || throwable.getCause() != null && throwable.getCause() instanceof UnsupportedOperationException) if (this.getServer().getBukkitVersion().startsWith("1.2.5")) Common.log(" &cSorry but Minecraft 1.2.5 is no longer supported!");
		 else {
		...
		}
		{
			...
			if (throwable.getMessage() != null && !throwable.getMessage().isEmpty() && !throwable.getMessage().equals("null")) error = throwable.getMessage();
	...
	public final void onDisable() {
		...
		try {
			for (final Player online : Remain.getOnlinePlayers()) SimpleScoreboard.clearBoardsFor(online);
		} catch (final Throwable t) {
		...
		try {
			for (final Player online : Remain.getOnlinePlayers()) {
				...
				if (menu != null) online.closeInventory();
			}
		} catch (final Throwable t) {
	...
	private void unregisterReloadables() {
		...
		try {
			if (HookManager.isDiscordSRVLoaded()) DiscordListener.clearRegisteredListeners();
	...
	protected final <T extends Listener> void registerAllEvents(final Class<T> extendingClass) {
		...
		for (final Class<? extends T> pluginClass : ReflectionUtil.getClasses(instance, extendingClass)) {
			...
			for (final Constructor<?> con : pluginClass.getConstructors()) if (con.getParameterCount() == 0) {
	...
	protected final void registerEvents(final Listener listener) {
		if (this.startingReloadables) this.reloadables.registerEvents(listener);
		 else this.getServer().getPluginManager().registerEvents(listener, this);
		if (listener instanceof DiscordListener) ((DiscordListener) listener).register();
	}
	...
	protected final <T extends Command> void registerAllCommands(final Class<T> extendingClass) {
		...
		for (final Class<? extends T> pluginClass : ReflectionUtil.getClasses(instance, extendingClass)) {
			// AutoRegister means the class is already being registered
			if (pluginClass.isAnnotationPresent(AutoRegister.class)) continue;
			if (SimpleSubCommand.class.isAssignableFrom(pluginClass)) continue;
			try {
				for (final Constructor<?> con : pluginClass.getConstructors()) if (con.getParameterCount() == 0) {
					final T instance = (T) ReflectionUtil.instantiate(con);
					if (instance instanceof SimpleCommand) this.registerCommand(instance);
					 else this.registerCommand(instance);
					continue classLookup;
				}
	...
	protected final void registerCommand(final Command command) {
		if (command instanceof SimpleCommand) ((SimpleCommand) command).register();
		 else Remain.registerCommand(command);
	}
	...
	protected final void registerCommands(final SimpleCommandGroup group) {
		if (this.startingReloadables) this.reloadables.registerCommands(group);
		 else group.register();
	}
