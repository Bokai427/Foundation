...
public abstract class SimpleConversation implements ConversationAbandonedListener {
	...
	public final void conversationAbandoned(final ConversationAbandonedEvent event) {
		...
		if (conversing instanceof Player) {
			...
			if (this.menuToReturnTo != null && this.reopenMenu()) {
				...
				if (title != null) Common.runLater(2, () -> newMenu.animateTitle(title));
			}
		}
	}
	...
	private final class CustomConversation extends Conversation {
		...
		private CustomConversation(final Conversable forWhom) {
			...
			if (SimpleConversation.this.insertPrefix() && SimpleConversation.this.getPrefix() != null) this.prefix = SimpleConversation.this.getPrefix();
		}

		@Override
		public void outputNextPrompt() {
			if (this.currentPrompt == null) try {
				...
				try {
					/*dont have expiring set class*/final ExpiringMap<String, Void> askedQuestions = (ExpiringMap<String, Void>) this.context.getAllSessionData().getOrDefault("Asked_" + promptClass, ExpiringMap.builder().expiration(SimpleConversation.this.getTimeout(), TimeUnit.SECONDS).build());
				...
				if (this.currentPrompt instanceof SimplePrompt) this.lastSimplePrompt = (SimplePrompt) this.currentPrompt;
