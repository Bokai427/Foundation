...
public final class MathUtil {
	...
	public static String toRoman(final int number) {
		...
		if (number == literal) return romanNumbers.get(number);
	...
	public static double average(final Double... values) {
		...
		double sum = 0;
		for (final double val : values) sum += val;
	...
	public static double calculate(final String expression) {
		...
		class Parser {
			...
			void eatSpace() {
				while (Character.isWhitespace(this.c)) this.eatChar();
			}

			double parse() {
				this.eatChar();
				final double v = this.parseExpression();
				if (this.c != -1) throw new CalculatorException("Unexpected: " + (char) this.c);
				return v;
			}
			...
			double parseFactor() {
				...
				if (this.c == '(') {
					...
					if (sb.length() == 0) throw new CalculatorException("Unexpected: " + (char) this.c);
