...
public class JSONParser {
	...
	private static JSONArray deserialize(final Reader deserializable, final Set<DeserializationOptions> flags) throws JSONParseException {
		...
		do {
			...
			default: 
				...
				default: 
					...
					} else throw new JSONParseException(lexer.getPosition(), JSONParseException.Problems.DISALLOWED_TOKEN, token);
					break;
				default: 
			...
			default: 
				...
				default: 
					...
					if (valueStack.size() > returnCount) valueStack.removeLast();
					 else 
		...
		(!(States.DONE.equals(currentState) && Yytoken.Types.END.equals(token.getType())));
	...
	public static Object deserialize(@NonNull final String deserializable) throws JSONParseException {
		if (deserializable == null) {
			throw new java.lang.NullPointerException("deserializable is marked non-null but is null");
		}
		final String trimmed = deserializable.trim();
		// Assume it's just a normal string
		if (!trimmed.startsWith("{") || !trimmed.endsWith("}")) return deserializable;
		Object returnable;
		...
		try {
			...
			if (readableDeserializable != null) readableDeserializable.close();
		}
		return returnable;
	}
	...
	public static String escape(final String escapable) {
		...
		for (int i = 0; i < characters; i++) {
			...
			default: 
				...
				if (((character >= '\000') && (character <= '\037')) || ((character >= '') && (character <= '')) || ((character >= ' ') && (character <= '⃿'))) {
	...
	private static States popNextState(final LinkedList<States> stateStack) {
		if (stateStack.size() > 0) return stateStack.removeLast();
		 else return States.PARSED_ERROR;
	}
	...
	public static void prettyPrint(final Reader readable, final Writer writable, final String indentation, final String newline) throws IOException, JSONParseException {
		...
		do {
			...
			default: 
				...
				for (int i = 0; i < level; i++) writable.append(indentation);
				break;
			default: 
				break;
			default: 
	...
	private static void serialize(final Object jsonSerializable, final Writer writableDestination, final Set<SerializationOptions> flags) throws IOException {
		...
		writableDestination.write(JSONParser.escape(jsonSerializable.toString()));
			...
			writableDestination.write("null");
			 else writableDestination.write(jsonSerializable.toString());
		} else if (jsonSerializable instanceof Float) {
			if (((Float) jsonSerializable).isInfinite() || ((Float) jsonSerializable).isNaN()) 
			/* Infinite and not a number are not supported by the JSON specification, so null is used instead. */
			writableDestination.write("null");
			 else writableDestination.write(jsonSerializable.toString());
		} else if (jsonSerializable instanceof Number) writableDestination.write(jsonSerializable.toString());
			...
			while (entries.hasNext()) {
				if (isFirstEntry) isFirstEntry = false;
				 else writableDestination.write(',');
				@SuppressWarnings("rawtypes")
			...
			writableDestination.write('[');
			for (int i = 0; i < numberOfElements; i++) if (i == (numberOfElements - 1)) JSONParser.serialize(writableArray[i], writableDestination, flags);
			 else {
