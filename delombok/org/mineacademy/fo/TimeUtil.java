...
public final class TimeUtil {
	...
	"(?:([0-9]+)\\s*y[a-z]*[,\\s]*)?" + "(?:([0-9]+)\\s*mo[a-z]*[,\\s]*)?" + "(?:([0-9]+)\\s*w[a-z]*[,\\s]*)?" + "(?:([0-9]+)\\s*d[a-z]*[,\\s]*)?" + "(?:([0-9]+)\\s*h[a-z]*[,\\s]*)?" + "(?:([0-9]+)\\s*m[a-z]*[,\\s]*)?" + "(?:([0-9]+)\\s*(?:s[a-z]*)?)?", Pattern.CASE_INSENSITIVE);
	...
	public static long toTicks(final String humanReadableTime) {
		...
		if (split.length < 2) throw new IllegalArgumentException("Expected human readable time like \'1 second\', got \'" + humanReadableTime + "\' instead");
		for (int i = 1; i < split.length; i++) {
			...
			if (sub.startsWith("tick")) isTicks = true;
			 else if (sub.startsWith("second")) unit = 1;
			 else if (sub.startsWith("minute")) unit = 60;
			 else if (sub.startsWith("hour")) unit = 3600;
			 else if (sub.startsWith("day")) unit = 86400;
			 else if (sub.startsWith("week")) unit = 604800;
			 else if (sub.startsWith("month")) unit = 2629743;
			 else if (sub.startsWith("year")) unit = 31556926;
			 else if (sub.startsWith("potato")) unit = 1337;
			 else throw new IllegalArgumentException("Must define date type! Example: \'1 second\' (Got \'" + sub + "\')");
	...
	public static String formatTimeColon(long seconds) {
		...
		minutes = minutes % 60;
		seconds = seconds % 60;
		return (days > 0 ? (days < 10 ? "0" : "") + days + ":" : "") + (hours > 0 ? (hours < 10 ? "0" : "") + hours + ":" : "") + (minutes > 0 ? (minutes < 10 ? "0" : "") + minutes + ":" : "00") + (seconds < 10 ? "0" : "") + seconds;
	}
	...
	public static long parseToken(String text) {
		...
		while (matcher.find()) {
			...
			for (int i = 0; i < matcher.groupCount(); i++) if (matcher.group(i) != null && !matcher.group(i).isEmpty()) {
				found = true;
				break;
			}
			if (found) {
				for (int i = 1; i < 8; i++) if (matcher.group(i) != null && !matcher.group(i).isEmpty()) {
		...
		if (!found) throw new NumberFormatException("Date not found from: " + text);
	...
	private static void checkLimit(String type, long value, int maxLimit) {
		if (value > maxLimit) throw new IllegalArgumentException("Value type " + type + " is out of bounds! Max limit: " + maxLimit + ", given: " + value);
	}
