...
public abstract class SimpleFlatDatabase<T> extends SimpleDatabase {
	...
	public final void load(final UUID uuid, final T cache, @Nullable Runnable runAfterLoad) {
		if (!this.isLoaded() || this.isQuerying) return;
		...
		Common.runAsync(() -> {
			try {
				...
				if (data == null || data.isEmpty()) {
					...
					if (Debugger.isDebugged("mysql")) Debugger.debug("mysql", "Data was empty, row has been removed.");
				} else if (this.isStored(uuid)) this.update("UPDATE {table} SET Data=\'" + data.toJson() + "\', Updated=\'" + System.currentTimeMillis() + "\' WHERE UUID=\'" + uuid + "\';");
				 else this.update("INSERT INTO {table}(UUID, Name, Data, Updated) VALUES (\'" + uuid + "\', \'" + name + "\', \'" + data.toJson() + "\', \'" + System.currentTimeMillis() + "\');");
				if (runAfterSave != null) Common.runLater(() -> runAfterSave.run());
	...
	private boolean isStored(@NonNull final UUID uuid) throws SQLException {
		...
		if (resultSet.next()) return resultSet.getString("UUID") != null;
		return false;
	}
