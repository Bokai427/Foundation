...
public final class ItemUtil {
	...
	public static String bountifyCapitalized(@NonNull CompChatColor color) {
		if (color == null) {
			throw new java.lang.NullPointerException("color is marked non-null but is null");
		}
	...
	public static String bountifyCapitalized(@NonNull Enum<?> enumeration) {
		if (enumeration == null) {
			throw new java.lang.NullPointerException("enumeration is marked non-null but is null");
		}
	...
	public static String bountify(@NonNull Enchantment enchant) {
		if (enchant == null) {
			throw new java.lang.NullPointerException("enchant is marked non-null but is null");
		}
	...
	public static boolean isSimilar(ItemStack first, ItemStack second) {
		...
		if ((firstAir && !secondAir) || (!firstAir && secondAir)) return false;
		if (firstAir && secondAir) return true;
		...
		if (!idMatch || !metaMatch || !(dataMatch || (dataMatch = first.getType() == Material.BOW))) return false;
		// ItemMeta
		{
			final ItemMeta f = first.getItemMeta();
			final ItemMeta s = second.getItemMeta();
			if ((f == null && s != null) || (s == null && f != null)) return false;
			if (f != null && s != null) {
				final String fName = Common.stripColors(f.getDisplayName());
				final String sName = Common.stripColors(s.getDisplayName());
				if ((fName != null && !fName.equals(sName)) || !listMatch(f.getLore(), s.getLore())) return false;
			}
		}
	...
	private static boolean listMatch(List<String> first, List<String> second) {
		if (first == null) first = new ArrayList<>();
		...
		for (int i = 0; i < first.size(); i++) {
			...
			if (!Common.stripColors(firstString).equals(Common.stripColors(secondString))) return false;
		}
		return true;
	}
	...
	private static boolean matchNbt(String key, NBTItem firstNbt, NBTItem secondNbt) {
		...
		if (!firstHas && !secondHas) return true; // nothing has, essentially same
		 else if (firstHas && !secondHas || !firstHas && secondHas) return false; // one has but another hasn't, cannot be same
