...
public final class BungeeUtil {
	...
	public static <T> void sendPluginMessage(Player sender, String channel, BungeeMessageType action, T... data) {
		...
		if (!action.name().equals("PLAYERS_CLUSTER_DATA")) Debugger.put("bungee", "Server \'" + Remain.getServerName() + "\' sent bungee message [" + channel + ", " + action + "]: ");
		if (sender == null) sender = findFirstPlayer();
		...
		for (Object datum : data) try {
			...
			if (datum instanceof CommandSender) datum = ((CommandSender) datum).getName();
			if (datum instanceof Integer) {
				...
				try {
					...
					if (t.getMessage().contains("too long")) Common.throwError(t, "Too long BungeeCord message to send (" + ((String) datum).length() + ")! Message: ", (String) datum);
					 else throw t;
				}
			...
			} else throw new FoException("Unknown type of data: " + datum + " (" + datum.getClass().getSimpleName() + ")");
			actionHead++;
		} catch (final Throwable t) {
			t.printStackTrace();
			return;
	...
	public static void connect(@NonNull Player player, @NonNull String serverName) {
		...
		if (serverName == null) {
			throw new java.lang.NullPointerException("serverName is marked non-null but is null");
		}
	...
	public static void sendBungeeMessage(@NonNull Player sender, Object... data) {
		...
		for (final Object datum : data) {
			...
			if (datum instanceof Integer) out.writeInt((Integer) datum);
			 else if (datum instanceof Double) out.writeDouble((Double) datum);
			 else if (datum instanceof Boolean) out.writeBoolean((Boolean) datum);
			 else if (datum instanceof String) out.writeUTF((String) datum);
			 else throw new FoException("Unknown type of data: " + datum + " (" + datum.getClass().getSimpleName() + ")");
		}
