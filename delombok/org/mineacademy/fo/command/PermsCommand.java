...
public final class PermsCommand extends SimpleSubCommand {
	...
	public PermsCommand(@NonNull Class<?> classToList) {
		...
		if (classToList == null) {
			throw new java.lang.NullPointerException("classToList is marked non-null but is null");
		}
	}
	...
	public PermsCommand(@NonNull Class<?> classToList, SerializedMap variables) {
		...
		if (!this.variables.containsKey("label") && SimplePlugin.getInstance().getMainCommand() != null) this.variables.put("label", SimplePlugin.getInstance().getMainCommand().getLabel());
	...
	private List<SimpleComponent> list(String phrase) {
		...
		try {
			do this.listIn(iteratedClass, messages, phrase);
 while (!(iteratedClass = iteratedClass.getSuperclass()).isAssignableFrom(Object.class));
	...
	private void listIn(Class<?> clazz, List<SimpleComponent> messages, String phrase) throws ReflectiveOperationException {
		...
		if (!messages.isEmpty() && !clazz.isAnnotationPresent(PermissionGroup.class)) throw new FoException("Please place @PermissionGroup over " + clazz);
		...
		for (final Field field : clazz.getDeclaredFields()) {
			...
			if (info.contains("{") && info.contains("}")) throw new FoException("Forgotten unreplaced variable in " + info + " for field " + field + " in " + clazz);
			...
			if (phrase == null || node.contains(phrase)) messages.add(SimpleComponent.of("  " + (has ? "&a" : "&7") + node + (def ? " " + Commands.PERMS_TRUE_BY_DEFAULT : "")).onClickOpenUrl("").onHover(Commands.PERMS_INFO + info, Commands.PERMS_DEFAULT + (def ? Commands.PERMS_YES : Commands.PERMS_NO), Commands.PERMS_APPLIED + (has ? Commands.PERMS_YES : Commands.PERMS_NO)));
		}
