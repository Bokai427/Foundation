...
public final class DebugCommand extends SimpleSubCommand {
		...
		for (final File file : files) try {
			...
			if (file.getName().endsWith(".yml")) {
				...
				for (final String key : config.getKeys(true)) {
					...
					if (!key.contains("MySQL")) copyConfig.set(key, value);
				}
				copyConfig.save(copy);
			} else Files.copy(file.toPath(), copy.toPath(), StandardCopyOption.REPLACE_EXISTING);
	...
	private List<File> listFilesRecursively(File folder, List<File> files) {
		for (final File file : folder.listFiles()) if (file.isDirectory()) {
			// Ignore log directory and ignore the debug directory itself
			if (!file.getName().equals("logs") && !file.getName().equals("debug")) this.listFilesRecursively(file, files);
		} else  // Ignore the debug zip file itself
		if (!file.getName().equals("debug.zip") && !file.getName().equals("mysql.yml")) files.add(file);
		return files;
	}
	...
	public static void addDebugLines(String... lines) {
		for (final String line : lines) debugLines.add(line);
	}
	...
	public static void setDebugLines(final List<String> debugLines) {
