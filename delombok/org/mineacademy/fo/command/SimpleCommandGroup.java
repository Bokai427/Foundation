...
public abstract class SimpleCommandGroup {
	...
	public final void register() {
		...
		if (this.aliases != null) this.mainCommand.setAliases(this.aliases);
	...
	private void checkSubCommandAliasesCollision() {
		...
		for (final SimpleSubCommand subCommand : this.subcommands) for (final String alias : subCommand.getSublabels()) {
	...
	protected final void registerSubcommand(final SimpleSubCommand command) {
		...
		if (this.subcommands.contains(command)) this.subcommands.remove(command);
	...
	protected List<SimpleComponent> getNoParamsHeader() {
		...
		messages.add(" ");
		{
			...
			if (!authors.isEmpty()) messages.add("   &7" + SimpleLocalization.Commands.LABEL_AUTHORS + " &f" + authors + (foundedYear != -1 ? " &7© " + foundedYear + (yearNow != foundedYear ? " - " + yearNow : "") : ""));
		}
		{
			final String credits = this.getCredits();
			if (credits != null && !credits.isEmpty()) messages.add("   " + credits);
		}
		messages.add("&8" + Common.chatLineSmooth());
	...
	public final class MainCommand extends SimpleCommand {
		...
		protected void onCommand() {
			...
			if (this.args.length == 0) {
				if (SimpleCommandGroup.this.sendHelpIfNoArgs()) this.tellSubcommandsHelp();
				 else this.tell(SimpleCommandGroup.this.getNoParamsHeader());
				return;
			}
			...
			if (!SimpleCommandGroup.this.getHelpLabel().isEmpty() && Valid.isInList(argument, SimpleCommandGroup.this.getHelpLabel())) this.tellSubcommandsHelp();
			 else this.returnInvalidArgs();
		}
		...
		protected void tellSubcommandsHelp() {
			...
			Common.runAsync(() -> {
				if (SimpleCommandGroup.this.subcommands.isEmpty()) {
					if (Messenger.ENABLED) this.tellError(SimpleLocalization.Commands.HEADER_NO_SUBCOMMANDS);
					 else Common.tell(this.sender, SimpleLocalization.Commands.HEADER_NO_SUBCOMMANDS);
					return;
				}
				final List<SimpleComponent> lines = new ArrayList<>();
				final boolean atLeast17 = MinecraftVersion.atLeast(V.v1_7);
				for (final SimpleSubCommand subcommand : SimpleCommandGroup.this.subcommands) if (subcommand.showInHelp() && this.hasPerm(subcommand.getPermission())) {
					...
					final String plainMessage = Replacer.replaceArray(SimpleCommandGroup.this.getSubcommandDescription(), "label", this.getLabel(), "sublabel", (atLeast17 ? "&n" : "") + subcommand.getSublabel() + (atLeast17 ? "&r" : ""), "usage", usage, "description", !desc.isEmpty() && !atLeast17 ? desc : "", "dash", !desc.isEmpty() && !atLeast17 ? "&e-" : "");
					...
					if (!desc.isEmpty() && atLeast17) {
						...
						if (subcommand.getPermission() != null) hover.add(SimpleLocalization.Commands.HELP_TOOLTIP_PERMISSION.replace("{permission}", subcommand.getPermission()));
						if (subcommand.getMultilineUsageMessage() != null && subcommand.getMultilineUsageMessage().length > 0) {
							...
							for (final String usageLine : subcommand.getMultilineUsageMessage()) hover.add("&f" + this.replacePlaceholders(this.colorizeUsage(usageLine.replace("{sublabel}", subcommand.getSublabel()))));
						} else hover.add(this.replacePlaceholders(SimpleLocalization.Commands.HELP_TOOLTIP_USAGE + (usage.isEmpty() ? command : usage)));
				...
				if (!lines.isEmpty()) {
					...
					if (SimpleCommandGroup.this.getHelpHeader() != null) pages.setHeader(SimpleCommandGroup.this.getHelpHeader());
		...
		private SimpleSubCommand findSubcommand(final String label) {
			for (final SimpleSubCommand command : SimpleCommandGroup.this.subcommands) {
				...
				for (final String alias : command.getSublabels()) if (alias.equalsIgnoreCase(label)) return command;
			}
			return null;
		}
		...
		public List<String> tabComplete() {
			if (this.args.length == 1) return this.tabCompleteSubcommands(this.sender, this.args[0]);
			if (this.args.length > 1) {
				final SimpleSubCommand cmd = this.findSubcommand(this.args[0]);
				if (cmd != null) return cmd.tabComplete(this.sender, this.getLabel(), Arrays.copyOfRange(this.args, 1, this.args.length));
			}
			return null;
		}
		...
		private List<String> tabCompleteSubcommands(final CommandSender sender, String param) {
			...
			for (final SimpleSubCommand subcommand : SimpleCommandGroup.this.subcommands) if (subcommand.showInHelp() && !(subcommand instanceof FillerSubCommand) && this.hasPerm(subcommand.getPermission())) for (final String label : subcommand.getSublabels()) if (!label.trim().isEmpty() && label.startsWith(param)) tab.add(label);
			return tab;
		}
	}
