...
public abstract class SimpleSubCommand extends SimpleCommand {
	...
	protected SimpleSubCommand(SimpleCommandGroup parent, String sublabel) {
		...
		if (this.getRawPermission().equals(getDefaultPermission())) if (SimplePlugin.getInstance().getMainCommand() != null && SimplePlugin.getInstance().getMainCommand().getLabel().equals(this.getLabel())) this.setPermission(this.getRawPermission().replace("{label}", "{sublabel}")); // simply replace label with sublabel
		 else this.setPermission(this.getRawPermission() + ".{sublabel}"); // append the sublabel at the end since this is not our main command
	}
	 ...
	 */
	@java.lang.SuppressWarnings("all")
	protected void setSublabel(final String sublabel) {
