...
public class ConfigSection {
	...
	ConfigSection(@NonNull ConfigSection parent, @NonNull String path) {
		if (parent == null) {
			throw new java.lang.NullPointerException("parent is marked non-null but is null");
		}
		if (path == null) {
	...
	public final void store(@NonNull String path, Object value) {
		...
		if (path.isEmpty()) throw new IllegalArgumentException("Cannot set to an empty path");
		if (this.root == null) throw new IllegalStateException("Cannot use section without a root");
		int leadingIndex = -1;
		int trailingIndex;
	...
	public final Object retrieve(@NonNull String path) {
		...
		if (this.root == null) throw new IllegalStateException("Cannot access section without a root");
		int leadingIndex = -1;
		int trailingIndex;
		ConfigSection section = this;
		while ((leadingIndex = path.indexOf('.', trailingIndex = leadingIndex + 1)) != -1) {
			final String currentPath = path.substring(trailingIndex, leadingIndex);
			if (section.retrieve(currentPath) == null) return null;
			section = section.retrieveConfigurationSection(currentPath);
			if (section == null) return null;
		}
		final String key = path.substring(trailingIndex);
		if (section == this) return this.map.get(key);
	...
	public final ConfigSection retrieveConfigurationSection(@NonNull String path) {
		...
		if (val != null) return (val instanceof ConfigSection) ? (ConfigSection) val : null;
	...
	final ConfigSection createSection(@NonNull String path) {
		...
		if (path.isEmpty()) throw new IllegalArgumentException("Cannot create section at empty path");
		if (this.root == null) throw new IllegalStateException("Cannot create section without a root");
		int leadingIndex = -1;
		int trailingIndex;
		ConfigSection section = this;
		while ((leadingIndex = path.indexOf('.', trailingIndex = leadingIndex + 1)) != -1) {
	...
	private void mapChildrenKeys(@NonNull Set<String> output, @NonNull ConfigSection section, boolean deep) {
		if (output == null) {
			throw new java.lang.NullPointerException("output is marked non-null but is null");
		}
		if (section == null) {
			throw new java.lang.NullPointerException("section is marked non-null but is null");
		}
		if (section instanceof ConfigSection) {
			...
			for (final String key : keys) output.add(createPath(section, key, this));
		}
	}
	...
	private void mapChildrenValues(@NonNull Map<String, Object> output, @NonNull ConfigSection section, boolean deep) {
		...
		if (section instanceof ConfigSection) {
			...
			for (final Map.Entry<String, Object> entry : sec.map.entrySet()) {
				...
				if (entry.getValue() instanceof ConfigSection) if (deep) this.mapChildrenValues(output, (ConfigSection) entry.getValue(), deep);
			}
		} else {
			final Map<String, Object> values = section.getValues(deep);
			for (final Map.Entry<String, Object> entry : values.entrySet()) output.put(createPath(section, entry.getKey(), this), entry.getValue());
		}
	}
	...
	private static String createPath(@NonNull ConfigSection section, String key, ConfigSection relativeTo) {
		...
		if (section != null) for (ConfigSection parent = section; (parent != null) && (parent != relativeTo); parent = parent.parent) {
	...
	public String toString() {
		...
		return new StringBuilder().append(this.getClass().getSimpleName()).append("[path=\'").append(this.fullPath).append("\', root=\'").append(root == null ? null : root.getClass().getSimpleName()).append("\']").toString();
	}
}
