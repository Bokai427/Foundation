...
final class YamlComments {
	...
	static void writeComments(@NonNull String jarPath, @NonNull File diskFile, @Nullable String oldContents, @NonNull List<String> ignoredSections) throws IOException {
		if (jarPath == null) {
			throw new java.lang.NullPointerException("jarPath is marked non-null but is null");
		}
		if (diskFile == null) {
			throw new java.lang.NullPointerException("diskFile is marked non-null but is null");
		}
		if (ignoredSections == null) {
			throw new java.lang.NullPointerException("ignoredSections is marked non-null but is null");
		}
		...
		try {
			if (oldContents != null) oldConfig.loadFromString(oldContents);
			 else oldConfig.load(diskFile);
		...
		for (final String ignoredSection : ignoredSections) if (newConfig.isSet(ignoredSection)) Valid.checkBoolean(newConfig.isConfigurationSection(ignoredSection), "Can only ignore config sections in " + jarPath + " (file " + diskFile + ")" + " not \'" + ignoredSection + "\' that is " + newConfig.get(ignoredSection));
		...
		for (final Map.Entry<String, Object> oldEntry : oldConfig.getValues(true).entrySet()) {
			...
			for (final String ignoredKey : ignoredSections) if (oldKey.startsWith(ignoredKey)) continue outerLoop;
			if (!newKeys.contains(oldKey)) removedKeys.put(oldKey, oldEntry.getValue());
		}
		...
		if (!removedKeys.isEmpty()) {
			...
			for (final Map.Entry<String, Object> entry : removedKeys.entrySet()) backupConfig.set(entry.getKey(), entry.getValue());
	...
	private static void write(FileConfiguration newConfig, FileConfiguration oldConfig, Map<String, String> comments, List<String> ignoredSections, BufferedWriter writer, Yaml yaml) throws IOException {
		...
		for (final String key : newConfig.getKeys(true)) {
			checkIgnore: {
				for (final String allowed : copyAllowed) if (key.startsWith(allowed)) break checkIgnore;
				for (final String allowed : copyAllowed) if (key.startsWith(allowed)) continue outerloop;
				// These keys are already written below
				for (final String allowed : reverseCopy) if (key.startsWith(allowed)) continue outerloop;
				for (final String ignoredSection : ignoredSections) {
					if (key.equals(ignoredSection)) {
						...
						if ((oldIgnoredObject instanceof String && oldIgnoredObject.toString().equals("{}")) || (oldIgnoredObject instanceof List && ((List<?>) oldIgnoredObject).isEmpty()) || (oldIgnoredObject instanceof MemorySection && ((MemorySection) oldIgnoredObject).getKeys(false).isEmpty())) {
						...
						// Write from old to new, copying all keys and subkeys manually
						{
							...
							for (final String oldKey : oldConfig.getConfigurationSection(ignoredSection).getKeys(true)) write0(ignoredSection + "." + oldKey, null, true, oldConfig, newConfig, comments, ignoredSections, writer, yaml);
							reverseCopy.add(ignoredSection);
							continue outerloop;
						}
					}
					if (key.startsWith(ignoredSection)) continue outerloop;
				}
			}
	...
	private static void write0(String key, @Nullable String forceWriteKey, boolean forceNew, FileConfiguration newConfig, FileConfiguration oldConfig, Map<String, String> comments, List<String> ignoredSections, BufferedWriter writer, Yaml yaml) throws IOException {
		...
		if (forceWriteKey != null) write(forceWriteKey, actualKey, prefixSpaces, yaml, writer);
		 else 
		// Write the old section
		if (newObj instanceof ConfigurationSection && !forceNew && oldObj instanceof ConfigurationSection) writeSection(writer, actualKey, prefixSpaces, (ConfigurationSection) oldObj);
		 else 
		// Write the new section, old value is no more
		if (newObj instanceof ConfigurationSection) writeSection(writer, actualKey, prefixSpaces, (ConfigurationSection) newObj);
		 else 
		// Write the old object
		if (oldObj != null && !forceNew) write(oldObj, actualKey, prefixSpaces, yaml, writer);
		 else 
		// Write new object
	...
	private static void write(Object obj, String actualKey, String prefixSpaces, Yaml yaml, BufferedWriter writer) throws IOException {
		if (obj instanceof ConfigurationSerializable) writer.write(prefixSpaces + actualKey + ": " + yaml.dump(((ConfigurationSerializable) obj).serialize()));
		...
		} else if (obj instanceof List) writeList((List<?>) obj, actualKey, prefixSpaces, yaml, writer);
		 else writer.write(prefixSpaces + actualKey + ": " + yaml.dump(obj));
	}
	...
	private static void writeSection(BufferedWriter writer, String actualKey, String prefixSpaces, ConfigurationSection section) throws IOException {
		if (section.getKeys(false).isEmpty()) writer.write(prefixSpaces + actualKey + ":");
		 else writer.write(prefixSpaces + actualKey + ":");
		writer.write("\n");
	}
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (o instanceof Map) {
				...
				for (final Map.Entry<?, ?> entry : ((Map<?, ?>) o).entrySet()) {
					...
					entryIndex++;
					if (entryIndex != mapSize) builder.append("\n");
				}
			} else if (o instanceof String || o instanceof Character) builder.append(prefixSpaces).append("- \'").append(o.toString().replace("\'", "\'\'")).append("\'");
			 else if (o instanceof List) builder.append(prefixSpaces).append("- ").append(yaml.dump(o));
			 else builder.append(prefixSpaces).append("- ").append(o);
			if (i != list.size()) builder.append("\n");
		}
		return builder.toString();
	}
	...
	private static Map<String, String> parseComments(List<String> lines, List<String> ignoredSections, FileConfiguration oldConfig, Yaml yaml) {
		...
		for (final String line : lines) {
			...
			if (line == null || line.trim().equals("") || line.trim().startsWith("#")) builder.append(line).append("\n");
			 else {
	...
	private static int countIndents(String s) {
		int spaces = 0;
		for (final char c : s.toCharArray()) if (c == ' ') spaces += 1;
		 else break;
		return spaces / 2;
	}
	...
	private static int setFullKey(StringBuilder keyBuilder, String configLine, int lastLineIndentCount) {
		...
		if (keyBuilder.length() == 0) keyBuilder.append(key);
		...
		keyBuilder.append(".").append(key);
			...
			for (int i = 0; i < difference + 1; i++) removeLastKey(keyBuilder);
			if (keyBuilder.length() > 0) keyBuilder.append(".");
			keyBuilder.append(key);
		}
