...
public class Region implements ConfigSerializable {
	...
	private Location[] getCorrectedPoints() {
		if (this.primary == null || this.secondary == null) return null;
		...
		final int x1 = this.primary.getBlockX();
		final int x2 = this.secondary.getBlockX();
		final int y1 = this.primary.getBlockY();
		final int y2 = this.secondary.getBlockY();
		final int z1 = this.primary.getBlockZ();
		final int z2 = this.secondary.getBlockZ();
	...
	public final Location getCenter() {
		...
		return new Location(primary.getWorld(), (primary.getX() + secondary.getX()) / 2, (primary.getY() + secondary.getY()) / 2, (primary.getZ() + secondary.getZ()) / 2);
	}
	...
	public final List<Entity> getEntities() {
		...
		for (int cx = xMin; cx <= xMax; ++cx) for (int cz = zMin; cz <= zMax; ++cz) for (final Entity entity : this.getWorld().getChunkAt(cx, cz).getEntities()) if (entity.isValid() && entity.getLocation() != null && this.isWithin(entity.getLocation())) found.add(entity);
		return found;
	}
	...
	public final World getWorld() {
		if (!this.isWhole()) return null;
		if (this.primary != null && this.secondary == null) return Bukkit.getWorld(this.primary.getWorld().getName());
		if (this.secondary != null && this.primary == null) return Bukkit.getWorld(this.secondary.getWorld().getName());
	...
	public final boolean isWithin(@NonNull final Location location) {
		...
		if (!location.getWorld().getName().equals(this.primary.getWorld().getName())) return false;
		final Location[] centered = this.getCorrectedPoints();
		final Location primary = centered[0];
		final Location secondary = centered[1];
	...
	public final boolean isWithinXZ(@NonNull final Location location) {
		...
		final int x = (int) location.getX();
		final int z = (int) location.getZ();
		return x >= primary.getX() && x <= secondary.getX() && z >= primary.getZ() && z <= secondary.getZ();
	}
	...
	public final void updateLocation(@Nullable Location primary, @Nullable Location secondary) {
		if (primary != null) this.setPrimary(primary);
		if (secondary != null) this.setSecondary(secondary);
	}
	...
	public boolean equals(Object obj) {
		if (obj instanceof Region) {
			...
			if ((otherRegion.name != null && this.name == null) || (otherRegion.name == null && this.name != null)) return false;
			if ((otherRegion.name != null && !otherRegion.name.equals(this.name)) || (otherRegion.name != null && !this.name.equals(otherRegion.name))) return false;
