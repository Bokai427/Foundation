...
public final class Debugger {
	...
	public static void debug(String section, String... messages) {
		if (isDebugged(section)) for (final String message : messages) print("[" + section + "] " + message);
	}
	...
	public static void saveError(Throwable t, String... messages) {
		...
		try {
			...
			fill(lines, "------------------------------------[ " + TimeUtil.getFormattedDate() + " ]-----------------------------------", header, systemInfo, "Plugins: " + Common.join(Bukkit.getPluginManager().getPlugins(), ", "), "----------------------------------------------------------------------------------------------");
			...
			}
			{
				...
				do {
					...
					for (final StackTraceElement el : t.getStackTrace()) {
						...
						if (count > 6 && trace.startsWith("net.minecraft.server")) break;
	...
	public static List<String> traceRoute(boolean trackLineNumbers) {
		...
		for (final StackTraceElement el : exception.getStackTrace()) {
			...
			if (line.contains("org.bukkit.plugin.java.JavaPluginLoader") || line.contains("org.bukkit.plugin.SimplePluginManager") || line.contains("org.bukkit.plugin.JavaPlugin")) continue;
			if (!paths.contains(className)) paths.add(className + "#" + el.getMethodName() + (trackLineNumbers ? "(" + el.getLineNumber() + ")" : ""));
		}
		// Remove call to self
	...
	public static void printValues(Object[] values) {
		if (values != null) {
			...
			for (int i = 0; i < values.length; i++) print("&8[" + i + "] &7" + values[i]);
		} else print("Value are null");
	}
	...
	public static void printStackTrace(String message) {
		...
		for (int i = 1; i < trace.length; i++) {
			...
			if (canPrint(line)) print("\tat " + line);
		}
		print("--------------------------------------------------------------------------------------------------------end-");
	}
 ...
 while ((cause = cause.getCause()) != null);
	...
	private static void print(String message) {
		if (Bukkit.getConsoleSender() != null) Bukkit.getConsoleSender().sendMessage(Common.colorize(message));
		 else System.out.println(Common.stripColors(message)); // our instance may or may not be available yet to log
	}
	...
	public static boolean isDebugModeEnabled() {
		return Debugger.debugModeEnabled;
	}
}
