...
public final class LagCatcher {
	...
	public static void start(String section) {
		if (SimpleSettings.LAG_THRESHOLD_MILLIS == -1) return;
	...
	public static void end(String section, int thresholdMs, String message) {
		...
		if (lag > thresholdMs && SimpleSettings.LAG_THRESHOLD_MILLIS != -1) {
			...
			if (printingMessages) System.out.println(message);
		}
	}
	...
	public static void performanceTest(int cycles, String name, Runnable code) {
		...
		if (!durationsMap.isEmpty()) {
			for (final Map.Entry<String, List<Long>> entry : durationsMap.entrySet()) {
				...
				long duration = 0;
				for (final long sectionDuration : entry.getValue()) duration += sectionDuration;
				System.out.println("\tSection \'" + section + "\' took " + MathUtil.formatTwoDigits(duration / 1000000.0));
			}
	...
	public static void took(String section) {
		...
		final String message = section + " took " + MathUtil.formatTwoDigits(nanoTime == null ? 0.0 : (System.nanoTime() - nanoTime) / 1000000.0) + " ms";
		if (printingMessages) if (SimplePlugin.hasInstance()) Common.logNoPrefix("[{plugin_name} {plugin_version}] " + message);
		 else System.out.println("[LagCatcher] " + message);
	}
	...
	private static double finishAndCalculate(String section) {
		...
		return nanoTime == null ? 0.0 : (System.nanoTime() - nanoTime) / 1000000.0;
	}
	 ...
	 */
	@java.lang.SuppressWarnings("all")
	public static void setPrintingMessages(final boolean printingMessages) {
		LagCatcher.printingMessages = printingMessages;
	}
	...
	public static boolean isPrintingMessages() {
		return LagCatcher.printingMessages;
	}
}
