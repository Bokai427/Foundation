...
public final class BlockUtil {
	 ...
	 */
	private static final Set<String> TREE_GROUND_BLOCKS = Sets.newHashSet("GRASS_BLOCK", "COARSE_DIRT", "DIRT", "MYCELIUM", "PODZOL");
	/**
	...
	public static boolean isWithinCuboid(final Location location, final Location primary, final Location secondary) {
		...
		if (locX >= x && locX <= x1 || locX <= x && locX >= x1) if (locZ >= z && locZ <= z1 || locZ <= z && locZ >= z1) if (locY >= y && locY <= y1 || locY <= y && locY >= y1) return true;
		return false;
	}
	...
	public static Set<Location> getBoundingBox(@NonNull Chunk chunk) {
		if (chunk == null) {
			throw new java.lang.NullPointerException("chunk is marked non-null but is null");
		}
	...
	public static Set<Location> getBoundingBox(final Location primary, final Location secondary) {
		...
		for (int i = 0; i < bottomCorners.size(); i++) {
			...
			for (double offset = BOUNDING_VERTICAL_GAP; offset < height; offset += BOUNDING_VERTICAL_GAP) {
				final VectorHelper p5 = p1.add(0.0, offset, 0.0);
				final VectorHelper p6 = p2.add(0.0, offset, 0.0);
				shape.addAll(plotLine(p5, p6));
			}
		}
		final Set<Location> locations = new HashSet<>();
		for (final VectorHelper vector : shape) locations.add(new Location(primary.getWorld(), vector.getX(), vector.getY(), vector.getZ()));
		return locations;
	}
	...
	public static Set<Location> getSphere(final Location location, final int radius, final boolean hollow) {
		...
		if (hollow) {
			for (int x = X - radius; x <= X + radius; x++) for (int y = Y - radius; y <= Y + radius; y++) for (int z = Z - radius; z <= Z + radius; z++) if ((X - x) * (X - x) + (Y - y) * (Y - y) + (Z - z) * (Z - z) <= radiusSquared) blocks.add(new Location(world, x, y, z));
	...
	public static Set<Location> getCircle(final Location location, final int radius, final boolean hollow) {
		...
		final int radiusSquared = radius * radius;
		if (hollow) {
			for (int x = initialX - radius; x <= initialX + radius; x++) for (int z = initialZ - radius; z <= initialZ + radius; z++) if ((initialX - x) * (initialX - x) + (initialZ - z) * (initialZ - z) <= radiusSquared) blocks.add(new Location(world, x, initialY, z));
	...
	private static Set<Location> makeHollow(final Set<Location> blocks, final boolean sphere) {
		...
		if (!sphere) {
			for (final Location location : blocks) {
				...
				if (!(blocks.contains(front) && blocks.contains(back) && blocks.contains(left) && blocks.contains(right))) edge.add(location);
			}
			return edge;
		}
		for (final Location location : blocks) {
			...
			if (!(blocks.contains(front) && blocks.contains(back) && blocks.contains(left) && blocks.contains(right) && blocks.contains(top) && blocks.contains(bottom))) edge.add(location);
		}
		return edge;
	}
	...
	public static List<Block> getBlocks(final Location primary, final Location secondary) {
		...
		for (int x = bottomBlockX; x <= topBlockX; x++) for (int z = bottomBlockZ; z <= topBlockZ; z++) for (int y = bottomBlockY; y <= topBlockY; y++) {
	...
	public static List<Block> getBlocks(@NonNull Chunk chunk) {
		...
		final int maxY = chunk.getWorld().getMaxHeight();
		final int maxZ = minZ | 15;
		for (int x = minX; x <= maxX; ++x) for (int y = 0; y <= maxY; ++y) for (int z = minZ; z <= maxZ; ++z) blocks.add(chunk.getBlock(x, y, z));
		return blocks;
	}
	...
	public static List<Block> getBlocks(final Location loc, final int height, final int radius) {
		...
		for (int y = 0; y < height; y++) for (int x = -radius; x <= radius; x++) for (int z = -radius; z <= radius; z++) {
			...
			if (checkBlock != null && checkBlock.getType() != Material.AIR) blocks.add(checkBlock);
		}
		return blocks;
	}
	...
	public static List<Chunk> getChunks(final Location location, final int radius) {
		...
		for (int x = chunkX - radius; x <= chunkX + radius; ++x) for (int z = chunkZ - radius; z <= chunkZ + radius; ++z) if (world.isChunkLoaded(x, z)) addedChunks.add(world.getChunkAt(x, z));
	...
	public static List<Location> getXZLocations(Chunk chunk) {
		...
		for (int x = chunkX; x < chunkX + 16; x++) for (int z = chunkZ; z < chunkZ + 16; z++) found.add(new Location(chunk.getWorld(), x, 0, z));
		return found;
	}
		...
		for (cycle = 0; cycle < 1000 && !toSearch.isEmpty(); cycle++) {
			...
			if (block.getType().toString().equals(logType) || block.getType().toString().equals(leaveType)) {
				...
				for (final BlockFace face : TREE_TRUNK_FACES) {
					...
					if (!searched.contains(relative)) toSearch.add(relative);
				}
			} else if (!block.getType().isTransparent()) return new ArrayList<>();
		}
	...
	public static boolean isLogOnGround(Block treeBaseBlock) {
		...
		if (!(CompMaterial.isLog(treeBaseBlock.getType()))) return false;
		// Reach for the bottom most tree-like block
		while (CompMaterial.isLog(treeBaseBlock.getType())) treeBaseBlock = treeBaseBlock.getRelative(BlockFace.DOWN);
	...
	public static boolean isBreakingFallingBlock(final Material material) {
		return material.isTransparent() && material != CompMaterial.NETHER_PORTAL.getMaterial() && material != CompMaterial.END_PORTAL.getMaterial() || material == CompMaterial.COBWEB.getMaterial() || material == Material.DAYLIGHT_DETECTOR || CompMaterial.isTrapDoor(material) || material == CompMaterial.OAK_SIGN.getMaterial() || CompMaterial.isWallSign(material) || 
	...
	public static boolean isTool(final Material material) {
		return  // axe & pickaxe
		// water, milk, lava,..
		material.name().endsWith("AXE") || material.name().endsWith("SPADE") || material.name().endsWith("SWORD") || material.name().endsWith("HOE") || material.name().endsWith("BUCKET") || material == CompMaterial.BOW.getMaterial() || material == CompMaterial.FISHING_ROD.getMaterial() || material == CompMaterial.CLOCK.getMaterial() || material == CompMaterial.COMPASS.getMaterial() || material == CompMaterial.FLINT_AND_STEEL.getMaterial();
	}
	...
	public static boolean isForBlockSelection(final Material material) {
		if (!material.isBlock() || material == Material.AIR) return false;
		try {
	...
	public static int findHighestBlockNoSnow(final World world, final int x, final int z) {
		for (int y = world.getMaxHeight() - 1; y > 0; y--) {
			...
			if (!CompMaterial.isAir(block)) {
				if (block.getType() == CompMaterial.SNOW_BLOCK.getMaterial()) return -1;
	...
	public static int findHighestBlock(final World world, final int x, final int z, final Predicate<Material> predicate) {
		...
		for (int y = world.getMaxHeight() - 1; y > minHeight; y--) {
			final Block block = world.getBlockAt(x, y, z);
			if (block != null && !CompMaterial.isAir(block) && predicate.test(block.getType())) return y + 1;
		}
		return -1;
	}
	...
	public static int findAirBlock(final World world, final int x, final int z, final boolean topDown, final Predicate<Material> predicate) {
		...
		if (topDown) for (int y = world.getMaxHeight() - 1; y > minHeight; y--) {
			final Block block = world.getBlockAt(x, y, z);
			if (block != null && !CompMaterial.isAir(block) && predicate.test(block.getType())) return y + 1;
		}
		 else for (int y = minHeight; y < world.getMaxHeight() - 1; y++) {
			final Block block = world.getBlockAt(x, y, z);
			final Block blockAbove = block.getRelative(BlockFace.UP);
			final Block blockTwoAbove = blockAbove.getRelative(BlockFace.UP);
			if (block != null && CompMaterial.isAir(blockAbove) && CompMaterial.isAir(blockTwoAbove) && predicate.test(block.getType())) return y + 1;
		}
		return -1;
	}
	...
	public static int findHighestNetherAirBlock(@NonNull World world, int x, int z) {
		...
		for (int y = 0; y < world.getMaxHeight(); y++) {
			...
			if (block != null && CompMaterial.isAir(block) && CompMaterial.isAir(above) && !CompMaterial.isAir(below) && below.getType().isSolid()) return y;
		}
		return -1;
	}
	...
	public static FallingBlock shootBlock(final Block block, final Vector velocity) {
		return shootBlock(block, velocity, 0.0);
	}
	...
	public static FallingBlock shootBlock(final Block block, final Vector velocity, final double burnOnFallChance) {
		if (!canShootBlock(block)) return null;
		final FallingBlock falling = Remain.spawnFallingBlock(block.getLocation().clone().add(0.5, 0, 0.5), block.getType());
		{
			...
			final double x = MathUtil.range(velocity.getX(), -2, 2) * 0.5;
			final double y = Math.random();
			final double z = MathUtil.range(velocity.getZ(), -2, 2) * 0.5;
			falling.setVelocity(new Vector(x, y, z));
		}
		if (RandomUtil.chanceD(burnOnFallChance) && block.getType().isBurnable()) scheduleBurnOnFall(falling);
		// Prevent drop
	...
	private static void scheduleBurnOnFall(final FallingBlock block) {
		EntityUtil.trackFalling(block, () -> {
			...
			if (upperBlock.getType() == Material.AIR) upperBlock.setType(Material.FIRE);
		});
	}
	...
	private static final class VectorHelper {
		...
		public boolean equals(final Object obj) {
			if (!(obj instanceof VectorHelper)) return false;
		...
		public VectorHelper(final double x, final double y, final double z) {
			this.x = x;
			this.y = y;
			this.z = z;
		}
