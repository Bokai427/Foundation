...
public enum CompAttribute {
	...
	public final Double get(final LivingEntity entity) {
		try {
			...
			try {
				...
				if (MinecraftVersion.equals(V.v1_8)) t.printStackTrace();
				return null;
			}
		}
	}
	...
	public final void set(final LivingEntity entity, final double value) {
		...
		try {
			...
			if (this == GENERIC_MAX_HEALTH) entity.setMaxHealth(value);
			 else try {
				if (this.hasLegacy()) this.setLegacy(entity, value);
			} catch (final Throwable t) {
				if (MinecraftVersion.equals(V.v1_8)) t.printStackTrace();
				if (t instanceof NullPointerException) throw new FoException("Attribute " + this + " cannot be set for " + entity);
			}
		}
	}
	...
	private CompAttribute(final String minecraftName) {
