...
final class NmsAccessor {
	...
	static {
		try {
			...
			} else if (MinecraftVersion.newerThan(V.v1_7)) addEntity = ReflectionUtil.getDeclaredMethod(ofcWorld, "addEntity", nmsEntity, SpawnReason.class);
			 else addEntity = ReflectionUtil.getNMSClass("World", "net.minecraft.world.level.World").getDeclaredMethod("addEntity", nmsEntity, SpawnReason.class);
		} catch (final ReflectiveOperationException ex) {
			ex.printStackTrace();
	...
	static Object addEntity(final World bukkitWorld, final Object nmsEntity, final SpawnReason reason) throws ReflectiveOperationException {
		...
		if (hasEntityConsumer) if (hasRandomizeData) return addEntity.invoke(bukkitWorld, nmsEntity, reason, null, false);
		 else return addEntity.invoke(bukkitWorld, nmsEntity, reason, null);
