...
public final class CompMonsterEgg {
	...
	public static ItemStack makeEgg(final EntityType type, final int count) {
		...
		// For older MC
		if (itemStack.getType().toString().equals("MONSTER_EGG")) itemStack = setEntity(itemStack, type);
		return itemStack;
	}
	...
	public static EntityType getEntity(final ItemStack item) {
		...
		if (MinecraftVersion.atLeast(V.v1_13)) type = getTypeFromMaterial(item);
		 else if (Remain.hasSpawnEggMeta()) type = getTypeByMeta(item);
		 else type = getTypeByData(item);
		if (type == null) type = getTypeByNbt(item);
		if (type == null && acceptUnsafeEggs) type = EntityType.UNKNOWN;
	...
	private static EntityType getTypeFromMaterial(final ItemStack item) {
		...
		try {
			...
			for (final EntityType all : EntityType.values()) if (all.getName() != null && all.getName().equalsIgnoreCase(name)) type = all;
		}
	...
	private static EntityType getTypeByData(final ItemStack item) {
		...
		if (type == null) {
			if (item.getDurability() != 0) type = DataMap.getEntity(item.getDurability());
			if (type == null && item.getData().getData() != 0) type = DataMap.getEntity(item.getData().getData());
		}
		return type;
	}
	...
	private static EntityType getTypeByNbt(@NonNull final ItemStack item) {
		...
		try {
			...
			if (tagCompound == null && acceptUnsafeEggs) return null;
			...
			if (MinecraftVersion.atLeast(V.v1_11) && idString.startsWith("minecraft:")) idString = idString.split("minecraft:")[1];
	...
	public static ItemStack setEntity(ItemStack item, final EntityType type) {
		...
		Valid.checkBoolean(CompMaterial.isMonsterEgg(item.getType()), "Item must be a monster egg not " + item);
		if (Remain.hasSpawnEggMeta()) item = setTypeByMeta(item, type);
		 else item = setTypeByData(item, type);
		return item;
	}
	...
	private static ItemStack setTypeByData(final ItemStack item, final EntityType type) {
		...
		} else if (!acceptUnsafeEggs) throw new FoException("Could not set monster egg to " + type);
		return item;
	}
...
final class DataMap {
	...
	private static Integer getKeyFromValue(final String value) {
		for (final Entry<Integer, String> e : map.entrySet()) if (e.getValue().equals(value)) return e.getKey();
		return null;
	}
