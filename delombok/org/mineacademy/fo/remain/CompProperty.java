...
public enum CompProperty {
	...
	public void apply(Object instance, Object key) {
		...
		if (method == null) this.applyLegacy(instance, key);
		 else try {
			ReflectionUtil.invoke(method, instance, key);
		} catch (final Throwable t) {
			if (MinecraftVersion.olderThan(V.values()[0])) this.applyLegacy(instance, key);
			 else 
	...
	private void applyLegacy(Object instance, Object key) {
		if (instance instanceof Entity) {
			...
			if (this == INVULNERABLE) nbtEntity.setInteger("Invulnerable", has ? 1 : 0);
			 else if (this == AI) nbtEntity.setInteger("NoAI", has ? 0 : 1);
			 else if (this == CompProperty.GRAVITY) nbtEntity.setInteger("NoGravity", has ? 0 : 1);
		}
		if (Remain.hasItemMeta() && instance instanceof ItemMeta) if (this == UNBREAKABLE) try {
			final boolean has = Boolean.parseBoolean(key.toString());
			...
			if (MinecraftVersion.atLeast(V.v1_8)) t.printStackTrace();
		}
	}
	...
	public boolean isAvailable(Class<?> clazz) {
		if (this.isAvailable.containsKey(clazz)) return this.isAvailable.get(clazz);
		return this.getMethod(clazz) != null;
	}

	// Automatically returns the correct getter or setter method for class
	private Method getMethod(Class<?> clazz) {
		if (this.isAvailable.containsKey(clazz) && !this.isAvailable.get(clazz)) return null;
	...
	private CompProperty(final Class<?> requiredClass, final Class<?> setterMethodType) {
