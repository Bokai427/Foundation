...
public final class BossBarInternals implements Listener {
	...
	private BossBarInternals() {
		if (MinecraftVersion.olderThan(V.v1_6)) this.entityClass = null;
		 else if (Remain.isProtocol18Hack()) this.entityClass = NMSDragon_v1_8Hack.class;
		 else if (MinecraftVersion.equals(V.v1_6)) this.entityClass = NMSDragon_v1_6.class;
		 else if (MinecraftVersion.equals(V.v1_7)) this.entityClass = NMSDragon_v1_7.class;
		 else if (MinecraftVersion.equals(V.v1_8)) this.entityClass = NMSDragon_v1_8.class;
		 else this.entityClass = NMSDragon_v1_9.class;
		if (MinecraftVersion.atLeast(V.v1_6)) {
			...
			if (Remain.isProtocol18Hack()) Common.runTimer(5, () -> {
	...
	public void onPluginDisable(final PluginDisableEvent event) {
		if (event.getPlugin().equals(SimplePlugin.getInstance())) this.stop();
	}

	// Removes bars from all players
	private void stop() {
		for (final Player player : Remain.getOnlinePlayers()) this.removeBar(player);
		this.players.clear();
		for (final int timerID : this.timers.values()) Bukkit.getScheduler().cancelTask(timerID);
		this.timers.clear();
	}
	...
	private void handleTeleport(final Player player, final Location loc) {
		if (!this.hasBar(player)) return;
		final NMSDragon oldDragon = this.getDragon(player, "");
		if (oldDragon instanceof NMSDragon_v1_9 || oldDragon == null) return;
		Common.runLater(2, () -> {
			if (!this.hasBar(player)) return;
	...
	public void setMessage(Player player, String message, float percent, CompBarColor color, CompBarStyle style) {
		Valid.checkBoolean(0.0F <= percent && percent <= 100.0F, "Percent must be between 0F and 100F, but was: " + percent);
		if (this.entityClass == null) return;
		if (this.hasBar(player)) this.removeBar(player);
		...
		if (color != null) dragon.barColor = color;
		if (style != null) dragon.barStyle = style;
	...
	public void setMessage(final Player player, String message, final int seconds, final CompBarColor color, final CompBarStyle style) {
		...
		this.timers.put(player.getUniqueId(), Common.runTimer(20, 20, () -> {
			...
			if (drag.getHealth() <= 1) {
				...
				this.cancelTimer(player);
			} else this.sendDragon(drag, player);
		}).getTaskId());
		this.sendDragon(dragon, player);
	}
	...
	public void removeBar(final Player player) {
		...
		if (dragon instanceof NMSDragon_v1_9) ((NMSDragon_v1_9) dragon).removePlayer(player);
		 else Remain.sendPacket(player, this.getDragon(player, "").getDestroyPacket());
		this.players.remove(player.getUniqueId());
		this.cancelTimer(player);
	}
	...
	private NMSDragon getDragon(final Player player, final String message) {
		if (this.hasBar(player)) return this.players.get(player.getUniqueId());
	...
	private NMSDragon addDragon(final Player player, final Location loc, final String message) {
		...
		if (dragon instanceof NMSDragon_v1_9) ((NMSDragon_v1_9) dragon).addPlayer(player);
		 else Remain.sendPacket(player, dragon.getSpawnPacket());
	...
	private Location getDragonLocation(Location loc) {
		...
		if (pitch >= 55) loc.add(0, -3, 0);
		 else if (pitch <= -55) loc.add(0, 3, 0);
		 else loc = loc.getBlock().getRelative(getDirection(loc), Bukkit.getViewDistance() * 16).getLocation();
		loc.subtract(0, 10, 0);
		return loc;
	}
	...
	public static BossBarInternals getInstance() {
		return BossBarInternals.instance;
	}
}
