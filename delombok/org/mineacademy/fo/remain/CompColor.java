...
public final class CompColor {
	...
	private static <T extends Enum<T>> T getEnum(final String newName, final String oldName, final Class<T> clazz) {
		...
		if (en == null) en = ReflectionUtil.lookupEnumSilent(clazz, oldName);
		return en;
	}
	...
	public static CompColor fromName(String name) {
		// Support HEX colors
		if (name.startsWith("#") && name.length() == 7) return new CompColor(Color.fromRGB(Integer.parseInt(name.substring(1, 3), 16), Integer.parseInt(name.substring(3, 5), 16), Integer.parseInt(name.substring(5, 7), 16)));
		name = name.toUpperCase();
		for (final CompColor comp : values()) if (comp.chatColor.toString().equals(name) || comp.dye.toString().equals(name) || comp.legacyName.equals(name)) return comp;
	...
	public static CompColor fromDye(final DyeColor dye) {
		for (final CompColor comp : values()) if (comp.dye == dye || comp.legacyName.equals(dye.toString())) return comp;
	...
	public static CompColor fromChatColor(final ChatColor color) {
		for (final CompColor comp : values()) if (comp.chatColor == color || comp.legacyName.equals(color.toString())) return comp;
	...
	public static CompColor fromChatColor(final CompChatColor color) {
		for (final CompColor comp : values()) if (comp.chatColor.name().equalsIgnoreCase(color.getName()) || comp.legacyName.equalsIgnoreCase(color.toString())) return comp;
	...
	public static ChatColor toColor(final DyeColor dye) {
		for (final CompColor c : CompColor.values()) if (c.getDye() == dye) return c.getChatColor();
	...
	public static CompMaterial toWool(final ChatColor color) {
		...
		if (comp == AQUA) return CompMaterial.LIGHT_BLUE_WOOL;
		if (comp == BLACK) return CompMaterial.BLACK_WOOL;
		if (comp == BLUE) return CompMaterial.BLUE_WOOL;
		if (comp == BROWN) return CompMaterial.BROWN_WOOL;
		if (comp == DARK_AQUA) return CompMaterial.CYAN_WOOL;
		if (comp == DARK_BLUE) return CompMaterial.BLUE_WOOL;
		if (comp == DARK_GRAY) return CompMaterial.GRAY_WOOL;
		if (comp == DARK_GREEN) return CompMaterial.GREEN_WOOL;
		if (comp == DARK_PURPLE) return CompMaterial.PURPLE_WOOL;
		if (comp == DARK_RED) return CompMaterial.RED_WOOL;
		if (comp == GOLD) return CompMaterial.ORANGE_WOOL;
		if (comp == GRAY) return CompMaterial.LIGHT_GRAY_WOOL;
		if (comp == GREEN) return CompMaterial.LIME_WOOL;
		if (comp == LIGHT_PURPLE) return CompMaterial.MAGENTA_WOOL;
		if (comp == PINK) return CompMaterial.PINK_WOOL;
		if (comp == RED) return CompMaterial.RED_WOOL;
		if (comp == WHITE) return CompMaterial.WHITE_WOOL;
		if (comp == YELLOW) return CompMaterial.YELLOW_WOOL;
	...
	public static List<ChatColor> getChatColors() {
		...
		for (final ChatColor color : ChatColor.values()) if (color.isColor() && !color.isFormat()) list.add(color);
		return list;
	}
