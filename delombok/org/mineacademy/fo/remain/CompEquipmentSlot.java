...
public enum CompEquipmentSlot {
	 ...
	 */
	OFF_HAND("OFF_HAND", "OFF_HAND"), HEAD("HEAD", "HELMET"), CHEST("CHEST", "CHESTPLATE"), LEGS("LEGS", "LEGGINGS"), FEET("FEET", "BOOTS");
	/**
	 * The localizable key
	 */
	...
	public void applyTo(LivingEntity entity, ItemStack item, @Nullable Double dropChance) {
		...
		default: 
			...
			if (dropChance != null && !lacksDropChance) equipment.setItemInHandDropChance(dropChance.floatValue());
			break;
		default: 
			Valid.checkBoolean(MinecraftVersion.atLeast(V.v1_9), "Setting off hand item requires Minecraft 1.9+");
			equipment.setItemInOffHand(item);
			if (dropChance != null && !lacksDropChance) equipment.setItemInOffHandDropChance(dropChance.floatValue());
			break;
		default: 
			equipment.setHelmet(item);
			if (dropChance != null && !lacksDropChance) equipment.setHelmetDropChance(dropChance.floatValue());
			break;
		default: 
			equipment.setChestplate(item);
			if (dropChance != null && !lacksDropChance) equipment.setChestplateDropChance(dropChance.floatValue());
			break;
		default: 
			equipment.setLeggings(item);
			if (dropChance != null && !lacksDropChance) equipment.setLeggingsDropChance(dropChance.floatValue());
			break;
		default: 
			equipment.setBoots(item);
			if (dropChance != null && !lacksDropChance) equipment.setBootsDropChance(dropChance.floatValue());
			break;
		}
	}
	...
	public static CompEquipmentSlot fromKey(String key) {
		...
		for (final CompEquipmentSlot slot : values()) if (slot.key.equals(key) || slot.bukkitName.equals(key)) return slot;
	...
	public static void applyArmor(LivingEntity entity, CompColor color) {
		applyArmor(entity, color, 0.0, new HashSet<>());
	}
	...
	public static void applyArmor(LivingEntity entity, CompColor color, Set<CompEquipmentSlot> ignoredSlots) {
		applyArmor(entity, color, 0.0, ignoredSlots);
	}
	...
	public static void applyArmor(LivingEntity entity, CompColor color, Double dropChance, Set<CompEquipmentSlot> ignoredSlots) {
		if (!ignoredSlots.contains(HEAD)) HEAD.applyTo(entity, ItemCreator.of(CompMaterial.LEATHER_HELMET).color(color).make(), dropChance);
		if (!ignoredSlots.contains(CHEST)) CHEST.applyTo(entity, ItemCreator.of(CompMaterial.LEATHER_CHESTPLATE).color(color).make(), dropChance);
		if (!ignoredSlots.contains(LEGS)) LEGS.applyTo(entity, ItemCreator.of(CompMaterial.LEATHER_LEGGINGS).color(color).make(), dropChance);
		if (!ignoredSlots.contains(FEET)) FEET.applyTo(entity, ItemCreator.of(CompMaterial.LEATHER_BOOTS).color(color).make(), dropChance);
	}
	...
	public static void applyArmor(LivingEntity entity, Type type) {
		applyArmor(entity, type, 0.0, new HashSet<>());
	}
	...
	public static void applyArmor(LivingEntity entity, Type type, Set<CompEquipmentSlot> ignoredSlots) {
		applyArmor(entity, type, 0.0, ignoredSlots);
	}
	...
	public static void applyArmor(LivingEntity entity, Type type, Double dropChance, Set<CompEquipmentSlot> ignoredSlots) {
		// Compatibility
		if (type == Type.NETHERITE && MinecraftVersion.olderThan(V.v1_16)) type = Type.DIAMOND;
		String name = type == Type.GOLD ? "GOLDEN" : type.toString();
		if (!ignoredSlots.contains(HEAD)) HEAD.applyTo(entity, CompMaterial.valueOf(name + "_HELMET").toItem(), dropChance);
		if (!ignoredSlots.contains(CHEST)) CHEST.applyTo(entity, CompMaterial.valueOf(name + "_CHESTPLATE").toItem(), dropChance);
		if (!ignoredSlots.contains(LEGS)) LEGS.applyTo(entity, CompMaterial.valueOf(name + "_LEGGINGS").toItem(), dropChance);
		if (!ignoredSlots.contains(FEET)) FEET.applyTo(entity, CompMaterial.valueOf(name + "_BOOTS").toItem(), dropChance);
	}
	...
	public static enum Type {
		...
		public static Type fromArmor(CompMaterial armorMaterial) {
			...
			Valid.checkBoolean(n.contains("LEATHER") || n.contains("CHAINMAIL") || n.contains("IRON") || n.contains("GOLD") || n.contains("DIAMOND") || n.contains("NETHERITE"), "Only leather to netherite armors are supported, not: " + armorMaterial);
			return Type.valueOf(n.split("_")[0]);
		}
	}

	@java.lang.SuppressWarnings("all")
	private CompEquipmentSlot(final String key, final String bukkitName) {
		this.key = key;
