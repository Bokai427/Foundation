...
public enum CompParticle {
	EXPLOSION_NORMAL("explode"), EXPLOSION_LARGE("largeexplode"), EXPLOSION_HUGE("hugeexplosion"), FIREWORKS_SPARK("fireworksSpark"), WATER_BUBBLE("bubble"), WATER_SPLASH("splash"), WATER_WAKE("wake"), SUSPENDED("suspend"), SUSPENDED_DEPTH("depthsuspend"), CRIT("crit"), CRIT_MAGIC("magicCrit"), SMOKE_NORMAL("smoke"), SMOKE_LARGE("largesmoke"), SPELL("spell"), SPELL_INSTANT("instantSpell"), SPELL_MOB("mobSpell"), SPELL_MOB_AMBIENT("mobSpellAmbient"), SPELL_WITCH("witchMagic"), DRIP_WATER("dripWater"), DRIP_LAVA("dripLava"), VILLAGER_ANGRY("angryVillager"), VILLAGER_HAPPY("happyVillager"), TOWN_AURA("townaura"), NOTE("note"), PORTAL("portal"), ENCHANTMENT_TABLE("enchantmenttable"), FLAME("flame"), LAVA("lava"), FOOTSTEP("footstep"), CLOUD("cloud"), REDSTONE("reddust"), SNOWBALL("snowballpoof"), SNOW_SHOVEL("snowshovel"), SLIME("slime"), HEART("heart"), BARRIER("barrier"), ITEM_CRACK("iconcrack_"), BLOCK_CRACK("blockcrack_"), BLOCK_DUST("blockdust_"), WATER_DROP("droplet"), ITEM_TAKE("take"), MOB_APPEARANCE("mobappearance"), 
	// MC 1.8.8+
	DRAGON_BREATH, END_ROD, DAMAGE_INDICATOR, SWEEP_ATTACK, FALLING_DUST, TOTEM, SPIT, SQUID_INK, BUBBLE_POP, CURRENT_DOWN, BUBBLE_COLUMN_UP, NAUTILUS, DOLPHIN, SNEEZE, CAMPFIRE_COSY_SMOKE, CAMPFIRE_SIGNAL_SMOKE, COMPOSTER, FLASH, FALLING_LAVA, LANDING_LAVA, FALLING_WATER, DRIPPING_HONEY, FALLING_HONEY, LANDING_HONEY, FALLING_NECTAR, SOUL_FIRE_FLAME, ASH, CRIMSON_SPORE, WARPED_SPORE, SOUL, DRIPPING_OBSIDIAN_TEAR, FALLING_OBSIDIAN_TEAR, LANDING_OBSIDIAN_TEAR, REVERSE_PORTAL, WHITE_ASH, LIGHT, DUST_COLOR_TRANSITION, VIBRATION, FALLING_SPORE_BLOSSOM, SPORE_BLOSSOM_AIR, SMALL_FLAME, SNOWFLAKE, DRIPPING_DRIPSTONE_LAVA, FALLING_DRIPSTONE_LAVA, DRIPPING_DRIPSTONE_WATER, FALLING_DRIPSTONE_WATER, GLOW_SQUID_INK, GLOW, WAX_ON, WAX_OFF, ELECTRIC_SPARK, SCRAPE;
	/**
		...
		try {
			...
			if (MinecraftVersion.atLeast(V.v1_7) && MinecraftVersion.olderThan(V.v1_13)) {
				...
				if (MinecraftVersion.equals(V.v1_7)) {
					...
					if (this.nmsEnumParticle == null) this.packetConstructor = null;
					 else this.packetConstructor = ReflectionUtil.getConstructor(packetClass, particleClass, Boolean.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Integer.TYPE, int[].class);
				}
			} else {
				this.nmsEnumParticle = null;
				this.packetConstructor = null;
			...
			} else this.bukkitEnumParticle = null;
	...
	public void spawn(Location location, Color color, float particleSize) {
		...
		if (atLeast1_13) location.getWorld().spawnParticle((Particle) this.bukkitEnumParticle, location, 1, 0, 0, 0, 0, new DustOptions(color, particleSize));
		 else {
			final float red = color.getRed() == 0 ? Float.MIN_VALUE : color.getRed() / 255.0F;
	...
	public void spawn(Player player, Location location, Color color, float particleSize) {
		Valid.checkBoolean(this == REDSTONE, "Can only send colors for REDSTONE particle, not: " + this);
		if (atLeast1_13) player.spawnParticle((Particle) this.bukkitEnumParticle, location, 1, 0, 0, 0, 0, new DustOptions(color, particleSize));
		 else {
			final float red = color.getRed() == 0 ? Float.MIN_VALUE : color.getRed() / 255.0F;
			final float green = color.getGreen() / 255.0F;
			final float blue = color.getBlue() / 255.0F;
	...
	public final void spawn(Location location, CompMaterial data) {
		...
		if (atLeast1_12) {
			if (atLeast1_13) location.getWorld().spawnParticle((Particle) this.bukkitEnumParticle, location, 1, data.getMaterial().createBlockData());
			 else location.getWorld().spawnParticle((Particle) this.bukkitEnumParticle, location, 1, data.getMaterial().getNewData(data.getData()));
		} else this.spawn(location, 0, 0, 0, 0, 1, 0, (int) data.getData());
	}
	...
	public void spawn(Location location, double offsetX, double offsetY, double offsetZ, double speed, int count, double extra, int... data) {
		...
		if (this.bukkitEnumParticle != null) {
			if (MinecraftVersion.atLeast(V.v1_13) && this == REDSTONE) location.getWorld().spawnParticle((Particle) this.bukkitEnumParticle, location, count, offsetX, offsetY, offsetZ, extra, new DustOptions(Color.RED, 1.0F));
			 else location.getWorld().spawnParticle((Particle) this.bukkitEnumParticle, location, count, offsetX, offsetY, offsetZ, extra, data);
			...
			for (final Player player : Remain.getOnlinePlayers()) Remain.sendPacket(player, packet);
		}
	}
	...
	public final void spawn(Player player, Location location, CompMaterial data) {
		Valid.checkBoolean(this == ITEM_CRACK || this == BLOCK_CRACK || this == BLOCK_DUST || this == FALLING_DUST, "Can only call particle spawn with data on crack or dust particles, not: " + this);
		if (atLeast1_12) {
			if (atLeast1_13) player.spawnParticle((Particle) this.bukkitEnumParticle, location, 1, data.getMaterial().createBlockData());
			 else player.spawnParticle((Particle) this.bukkitEnumParticle, location, 1, data.getMaterial().getNewData(data.getData()));
		} else this.spawn(player, location, 0, 0, 0, 0, 1, 0, (int) data.getData());
	}
	...
	public void spawn(Player player, Location location, double offsetX, double offsetY, double offsetZ, double speed, int count, double extra, int... data) {
		// Minecraft 1.12 and up
		if (this.bukkitEnumParticle != null && this != REDSTONE) player.spawnParticle((Particle) this.bukkitEnumParticle, location, count, offsetX, offsetY, offsetZ, extra, data);
		 else if (this.packetConstructor != null) Remain.sendPacket(player, this.preparePacket(location.getX(), location.getY(), location.getZ(), offsetX, offsetY, offsetZ, speed, count, extra, data));
	}
		...
		try {
			if (atLeast1_8) {
				if (this == BLOCK_CRACK) {
					int materialId = 0;
					int materialData = 0;
					if (data.length > 0) materialId = data[0];
					if (data.length > 1) materialData = data[1];
