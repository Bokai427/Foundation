...
public final class CompMetadata {
	...
	public static void setMetadata(final Entity entity, final String key, final String value) {
		...
		if (Remain.hasScoreboardTags()) {
			...
			if (!entity.getScoreboardTags().contains(tag)) entity.addScoreboardTag(tag);
		} else {
	...
	public static String getMetadata(final ItemStack item, final String key) {
		...
		if (item == null || CompMaterial.isAir(item.getType())) return null;
	...
	public static String getMetadata(final Entity entity, final String key) {
		Valid.checkNotNull(entity);
		if (Remain.hasScoreboardTags()) for (final String line : entity.getScoreboardTags()) {
	...
	public static boolean hasMetadata(final ItemStack item, final String key) {
		...
		if (CompMaterial.isAir(item.getType())) return false;
		final NBTItem nbt = new NBTItem(item);
	...
	public static boolean hasMetadata(final Entity entity, final String key) {
		Valid.checkNotNull(entity);
		if (Remain.hasScoreboardTags()) for (final String line : entity.getScoreboardTags()) if (hasTag(line, key)) return true;
	...
	public static void removeTempMetadata(final Entity player, final String key) {
		if (player.hasMetadata(key)) player.removeMetadata(key, SimplePlugin.getInstance());
	}
	...
	public static final class MetadataFile extends YamlConfig {
		...
		protected void addMetadata(final BlockState blockState, final String key, final String value) {
			synchronized (LOCK) {
				...
				{
					// Save
					for (final Map.Entry<Location, BlockCache> entry : this.blockMetadataMap.entrySet()) this.set("Block." + SerializeUtil.serializeLoc(entry.getKey()), entry.getValue().serialize());
					this.save();
				}
			}
		}
		...
		public static final class BlockCache implements ConfigSerializable {
			...
			@java.lang.SuppressWarnings("all")
			public BlockCache(final CompMaterial type, final List<String> metadata) {
				this.type = type;
